...
- (Behind feature flag) Fixed UI inconsistency when adding accounts in full-page account list mode - actions now appear as a (#24468)
  bottom sheet overlay
- Added replaces active tab if max tabs are open and request comes from trending (#24555)
- Improved browser experience by hiding the app navigation bar when browsing and redesigned the browser controls with (#24444)
  quick-access buttons for reloading pages, managing bookmarks, and
  opening new tabs, plus added close and tabs buttons to the URL bar
...
- Updated Networks bottom sheet to open at 50% screen height with improved header and close button (#24493)
- Refactored mUSD conversion CTA rendering conditions (#24335)
- UseTokenAsset to make use of `@assets-controller` `getNativeTokenAddress` implementation. (#24483)
- Added a "Give feedback" button to Perps home screen (#24511)
- Hide Earn product entry points for ineligible users (#24490)
- Add 5 min polling to trending tokens (#24462)
- Adds intent based swap support (#23776)
- Improved Card onboarding flow with better state handling and navigation after delegation setup (#24430)
- Added perps deeplinks for home view, market list with category filtering, and HIP-3 asset support (#24132)
- Set trending token as the dest token when navigating to swaps (#24373)
- Misc mUSD conversion flow design changes ahead of UAT (#24336)
- Perps icon fallback url mechanism (#24340)
- Added fill type indicators (Take Profit, Stop Loss, Liquidation, ADL) to perps transactions on Home and Market Details (#24259)
  screens
- Allow metal card rewards claim when season 1 ends (#24314)
- Ai workflow with jira and simulator setup (#24283)
- feat: bip39 word bar suggestions in onboarding and import srp module. (#22927)
- Added a fallback for Perps users who have Basic Functionality disabled. (#23952)
- Added remotely configurable allowlist and blocklist for mUSD conversion tokens. (#24126)
- Use optimistic approach when updating TP/SL in perps (#24159)
- Added OTA updates modal (#24175)
- Updated token list item musd conversion cta position to secondary balance position. This replaces the price change percentage (#24249)
  for specified stablecoins
- Added a verification step to the Card onboarding flow to validate Veriff KYC results before proceeding. (#24246)
- Added override functionality to remote feature flags with methods to set and clear overrides and access A/B test groups (#23487)
- Improved initial cross ecosystem connection flows by preselecting all supported chains (EVM + Solana) when connecting through (#24137)
  injected providers.
- Implement modal for user already has a Card account (#24007)
- Added card-home deeplink handler to navigate users to MetaMask Card based on authentication and card-linked account status (#24118)
- Fixed HIP-3 market symbols displaying with DEX prefix in Order Book view (#24068)
- Added ability to view trade details when tapping on recent trades in Perps, with a "Trade again" button to quickly navigate (#24067)
  back to trading
- Added new card-onboarding deeplink to navigate users to the MetaMask Card feature (#24042)
- Reveal SRP flow ui and text updates. (#24058)
- Fixed "See all" button visibility in Perps Recent Activity section (#24099)
...
- Update optin metrics screen ui changes (#24008)
- NFTs allow fetching one page and aborting fetch when navigating away (#23962)
- Removed the notification badge from the card icon button (#23978)
- Improved Toast component visual alignment and positioning (#23928)
- Updated Deposit page transition to slide in from the right instead of bottom (#23913)
- Updated NFT details pages to slide in from right instead of from bottom (#23912)
- Changed DeFi protocol detail page to slide in from right instead of from bottom (#23911)
- Added QuickNode fallback RPC for Monad. (#23479)
- Added paste button for email verification code input on Deposit flow (#23939)
- Automatically change the destination asset to match source asset chain when users land on swaps. (#23822)
- Added useTronStakeApy hook (#23743)
- Added support for delegating Base assets in the MetaMask Card feature (#23509)
- Implement MM Travel and ToS links on Card Home (#23550)
- Disable token selection in Perps and Predict deposits if no native balance and gas station not supported (#23680)
- Updated the mUSD confirmation screen's header to display the mUSD token and target network icon. (#23691)
- Added mUSD CTA to asset overview screen (#23562)
- Add support for progressive rollout FF (#23670)
- Adds "Inaccurate fee" alert in Confirmations (#23588)
- Fixed Perps tab to display total balance instead of available balance (#23466)
- Add conditional rendering of bridge-time-row by tx type (#23552)
- Adds 1-click approval/deposit transaction confirmation flow for Earn lending (#23154)
- Add mUSD conversion CTA above token list (#23390)
- Fixed an issue where users could accidentally add burn addresses to their contacts, which could lead to sending funds to (#23358)
  unrecoverable addresses
- Social Login user create wallet will always enable metametric flag (#22469)
  SRP user create wallet will always disable metametric
  flag by default and prompted metametric screen
- Add support for gas-included swaps with EIP-7702 (#22593)
- Added MegaETH Mainnet to Additional Networks (PopularList) (#23401)
- Added new simplified predict market row item for trending (#23388)
- Network connection banner now hides immediately when network becomes available again (#23425)
- Get marketData on trending search request (#23197)
- Added automatic saving and restoration of trade form configuration when navigating away from the perps trading screen (#23321)
- Add mUSD conversion education screen on first visit to conversion flow (#22972)
- Added price deviation warning to Perps trading interface to prevent opening positions when perps price deviates (#23242)
...
- Show custom error msg page when user searches for token not found on trending page (#24569)
- Fixed a bug where TextField components could wrap text to multiple lines even when multiline={false} (#24584)
- Adds configurable minimum fiat balance required for tokens to be eligible for mUSD conversion flow and CTAs (#24532)
...
- Fixed QR code scanning in dark mode by adding white background (#24533)
- Fix incorrect skeleton used for predictions when searching in main explore page (#24519)
- Fix trending tokens search crashes on search (#24517)
- Explore browser tab open behaviour when opening empty vs populated browser (#24371)
- Use preferred search engine on trending (#24513)
- Add fallback for cardFeature flag using local value (#24428)
- Use custom values for liquidity and min14hvalue param for trending requests (#24481)
- fix: search token input box size (#24380)
- Fixed sentence case violations in locale strings (#24451)
- Add MetaMask Portfolio to explore sites (#24369)
- Reset token icon when source changes to recalculate fallback (#24334)
- Fixed a bug where total volume was displaying the wrong value in asset details page (#24331)
- Fix Predict feed animations, tab bar UI and search overlay (#24337)
- Using v3 api to get historical prices for asset details page instead of v1 (#24315)
- Bug fix where a user couldn't scroll through the SRP reveal bottom sheet on Android (#24338)
- Removes `Add account` in the account selector during the ramps flow. (#22114)
- Fixed an issue where Perps withdrawal indicators could remain stuck in "pending" state after the withdrawal completed (#24214)
- Fix trending search not getting market data (#24311)
- Token 2022 send (#24350)
- Fixed TP/SL price input validation to respect HyperLiquid's 5 significant figure limit (#24169)
- Fixes an issue where flipping source/dest assets do not update balances when user has zero balance on non-evm chains (#24329)
- Add conditional mounting to perps tab in Activity View (#24271)
- Add hybrid REST/websocket approach to building recent activity (#24212)
- Added ScreenshotDeterrent to the login page to prevent screenshots and recordings on the page. (#24285)
- Improved Card performance by reducing redundant authentication token refresh requests and ensuring CardHome updates (#24295)
  correctly after changing the spending asset or spending limit.
- Updated Edit Account Name to display as a full-page drill-down with right-to-left transition and back button instead of a (#23579)
  bottom sheet modal
- Fixed Swap button not showing for trending tokens on asset details page (#24299)
- Prevent filtering of transactions with 0x0 hash in activity view (#24289)
- Fixes a small localization key bug in the OTP code screen for Buy (deposit) (#24264)
- Modify the android manifest so that android 12+ will never ask for location permission. (#23759)
- Fixes issue where certain search queries in Predict would return no results (#24266)
- Fixed UI styling inconsistencies in Perps close positions and market balance views (#24234)
- Fixed inconsistent font size for TP/SL text in Perps position card (#24238)
- Fixed analytics `ramp_type` to correctly report `UNIFIED_BUY` when Unified Ramp V1 is enabled (#24245)
- Phishing screen UX background color papercut (#23836)
- Use cache headers on sentinel calls (#23429)
- Fixed mUSD conversion flow not rejecting transactions when navigating away (#24171)
- Fixed inconsistent price formatting on TradingView chart y-axis that showed unnecessary trailing zeros (#24184)
- Fixed rounded corners in Perps order screen when there are more than 2 rows displayed (#24162)
- Fixed payment method selector modal scroll not working (#24188)
- Fixed liquidation distance showing 0% instead of fallback when liquidation price is unavailable (#24128)
- Fixed sentence case violations in English locale strings lines 6001-7000 (#24056)
- Fixed market name text being truncated in Perps header for hip-3 markets (#24140)
- Fixed position size USD display showing 3 decimals instead of 2 (#24131)
- Added indicator name shortcuts in Perps asset pills to reflect sorting mechanism (#24130)
- Fixed sentence case violations in English locale strings lines 7001-7279 (#24127)
- Fixed bug where requesting quotes for successive non-EVM networks failed (#24095)
- Fixes header text in token details page (#24101)
- Fix balance updates using the new feature flags for balance fetching (#24156)
- Fixed sentence case violations in English locale strings lines 4001-5000 (#23996)
- Fixed sentence case violations in English locale strings lines 5001-6000 (#24049)
- Fixed delay between the live candlestick close price and current price line on the Perps TradingView chart (#24100)
- Fixed date format display on Perps price chart x-axis from "6 Nov" to "11/6" format (#24103)
- Fixed entry price decimal precision in Perps trade history (#24096)
- Fox animation raise up issue during keyboard open. (#23354)
- Fixed Perps activity view to show aggregated PnL values for stop loss and take profit orders that execute as multiple fills (#24050)
- Add excludeLabels to trending api. (#23988)
- Fix biometric sudden reset issue (#24018)
- fix: resolved minor send flow UI issues (#24034)
- Fixed sentence case violations in English locale strings lines 2001-3000 (#23957)
- Fix non-live current price in leverage slider (#24046)
- Fixed OHLC bar text overflow for tokens with small prices (#23471)
- Fixed an issue where pending approval requests could become permanently stuck in the queue, preventing users from approving (#24040)
  or rejecting subsequent connection requests.
- Avoid re-rendering home when navigating back from trending (#24062)
- Improved the minimum received bridge label by rounding down (#23851)
- Fix TokenListController initialization. (#24020)
- Fixed sentence case violations in English locale strings lines 3001-4000 (#23994)
- Remove ens calls from account list to improve perf (#23920)
- Fixed an issue where switching back to the native token after automatic gas fee token selection did not trigger the insufficient (#23855)
  balance alert.
- Fixed balance display in Perps to show proper decimal formatting with trailing zeros (#23898)
- Fixes empty quote card details issue when swapping on Solana network (#23915)
- Fixed a bug where alert modal styling would not update correctly when switching between different severity alerts. (#23893)
- Fixed a bug that caused Swap button to be blocked even though a quote was available (#23792)
- fix: update asset details transition to left to right (#23933)
- Token details block explorer correct urls (#23890)
- Google login fallback in android if no google account available on device. (#23520)
- Reduced number of calls to bulk-scan for NFT detection (#23803)
- Add default blockexplorer for linea and mainnet. (#23861)
- Resetpassword screen touch-id state (#23798)
- Remove emojis from gas options (#23907)
- fix: remove background from more button in multichain account selector (#23904)
- Change selected account styles (#22203)
- Remove scroll on click when clicking a tx in the asset details modal (#23885)
- Fixed "Report a detection problem" button to link to the correct GitHub repository for reporting phishing detection issues (#23824)
- fix: updated checkbox border to correct color (#23846)
- Fixed hiding nft flow. (#23833)
- Improved import assets UI (#23735)
- Fix nft auto detection not triggered on network switch (#23858)
- Fix token balances init (#23668)
- Fix issue where rejecting a MMConnect confirmation results in a connection failure toast (rather than a error toast) in the (#23806)
  wallet.
- Fix stable coin chart (#23168)
- Fix re-rendering of nfts grid (#23768)
- Fixed QR scanner navigation by chain type and recipient address pre-population in send flow (#21498)
- Fixed leverage initialization when modifying existing perpetual positions (#23619)
- Fixed a bug where stale quote data remained momentarily when changing swap destination token (#23737)
- Fixed bug where the EVM addresses were not checksummed (#23703)
- Fixed chart jumping issues when font accessibility is turned on (#23732)
- Updated swaps network picker ordering (#23686)
- fix: update bg color for earn upsell banner (#23701)
- fix: unified keyboard actions background color (#23649)
- Fixed sentence case violations in English locale strings lines 1-1000 (#23499)
- Fixed a flicker where the insufficient balance alert appeared before gas-station checks completed (#23361)
- fix: update lend UI (#23633)
- fix: enlarge question mark icon in lend header (#23635)
- Added readme for using OTA updates on nightly builds (#23573)
- Fixes an issue where when users selected the swap button for an asset presented in browser URL bar, that asset was not (#23534)
  selected as source when navigating to swap page.
- Hides trending section if empty (#23586)
- Fixed withdrawal progress component to display decimal amounts correctly (e.g., $1.30 instead of $1) (#23477)
- Truncate long titles in Predict claim confirmations (#23462)
- Fixed a bug where mainnet where showing after testnets in the send modal screen (#23492)
- Fixed sentence case violations in English locale strings lines 1001-2000 (#23516)
- Improved Portfolio integration by passing tracking consent from Mobile app (#22683)
- Removes usePopularNetworks hook and replace it with constant. (#23525)
- Google One Tap "user disabled the feature" errors are no longer sent to Sentry (#23101)
- Fixed an issue where "Fund your wallet" empty state briefly appeared when importing wallets with existing funds or switching (#23351)
  between funded accounts
- Fixed internal transfers not appearing in perps transaction history (#23405)
- Filter and handle unsupported notifications (#23291)
...
- fix: include hip3 dexes when building position map on init (#24300)
- chore(perps): Refactor HL subscription service (#24274, #24015)
...
- chore: update @metamask/keyring-api and @metamask/tron-wallet-snap versions (#24205)
- chore: adds localization method and other improvements (#24204)
- chore: add support for fiat use in tron staking/unstaking flows (#24199)
- chore: adds the Tron staking details preview (#24198)
- fix(ramp): use ScrollView from react-native-gesture-handler in PaymentMethodSelectorModal (#24197)
- chore: broadcast staking TRX transaction cp-7.61.3 (#24196)
...
- feat: MUSD-108: Restrict payment tokens during mUSD conversion flow (#23216)
- feat: Remove nonce editing from Settings (#23031)
- feat: dark-theme friendly cronos logo (#23323)
- feat: render gas fees sponsored network and swap (#20878)
- feat: trust signals alerts on addresses and urls (#23167)
- feat: add trust signal middlware (#22800)
- feat: malicious token screening on transactions (#22688)
- feat: basic functionality toggle for trending (#23252)
- feat: track RPC update from network connection banner (#22879)
- feat: no point estimation when no active season (#23244)
- feat: [Trending] added support for refreshing + some code restructuring (#23250)
- feat: Throw error in Segment instead of Sentry Stop throwing error in Sentry when user tries to add a chain with unrecognized rpc url (#23075)
- feat: Add predefined recipient support to send flow (#23087)
- feat: add trending tokens search (#23036)
- feat: track analytics events for View All button clicks in NFT and Token lists (#23107)
- feat: sites section (#23163)
- feat: Bump deflation numbers to latest migration cp-7.60.0 (#23161)
- feat: enhance PerpsHomeView with new empty state and balance actions (#23086)
- feat: musd conversion optimizations (#23146)
- feat: [Trending] allow navigating to a website or search on google using the omnisearch and other minor improvements (#22872)
- feat: add musd conversion flow (#23060)
- feat: improve Predict withdrawal toast messaging (#23045)
- feat: add sentry logs for OTA updates (#22682)
- feat: refactor and fix invalid RPC URL error on paste (#22875)
...
- feat: Enhance AccountSelector with full-page layout and animations (#22797)
- feat: Add skeleton loading for the Send flow (#22853)
- feat: migrate features to new arch (#22675)
- feat: brwoser button (#22871)
- feat: cp-7.60.0 add new networks logo (#22929)
- feat: Open OS settings when push notifications are disabled (#22830)
- feat: allow add account predict flow (#22856)
- feat: Implement New migration failure detection cp-7.60.0 (#22757)
- feat: Start implement base link handlers for new universal link-router system (#22566)
- feat: cp-7.60.0 validate source amount in metamask pay (#22758)
- feat: introduce quick pick options in swap keypad (#22407)
- feat: allow add account in swap flow (#22718)
- feat: [Trending] make sections dynamic and restructure code part 2/2 (#22724)
- feat: Integrate season status hook in PerpsHeroCardView (#22768)
- feat: clicking src/dest swap flipper should invert token amounts from previosuly fetched quote (#22540)
- feat: EAS updates feature flag (#22036)
...
- feat: upgrade notification package and add Platform Notifications (#22539)
...
- feat: mcwp176 add auto tracking for related feature flags and override (#22023)
- feat: implement view all prediction (#22621)
- feat: update metamask pay same chain duration (#22629)
- feat: Implement `onAmountInput` for nonEVM send flow amount validations (#22389)
...
- feat: Implement part 1 of UniversalRouter for link-handling (deep links) (#22453)
...
- feat: update predict claim background (#22348)
- feat: support importing and ignoring non-evm tokens (#22103)
- feat: add logout button to BasicInfo error banner for Transak phone already registered error (#22183)
- feat: Implement LegacyLinkAdapter between old and new 'core' links (#22202)
- feat: calculate insufficientBal for quote requests (#22312)
...
- fix: skip expo updates configuration in env e2e (#23337)
...
- fix: skip Snap account name (#23221)
- fix: SL-325: remove runAfterInteraction (#22653)
- fix: cp-7.60.0 remove max button from metamask pay deposits (#23287)
- fix: rework right back button in send flow when navigating from asset overview (#23144)
- fix: Fix MM Connect resuming of sessions before NetworkController state is available for BackgroundBridge (#22749)
...
- fix: cp-7.60.0 set default balances polling (#23253)
- fix: cp-7.60.0 predict withdraw using gas station (#23255)
- fix: cp-7.60.0 alerts persisting in metamask pay (#23240)
- fix: cp-7.60.0 gas station with metamask pay if send bundle supported (#23219)
- fix: revert version number (#23217)
- fix: cp-7.60.0 perps deposit activity filtering (#23158)
- fix: cp-7.60.0 predict withdraw value in activity (#23156)
- fix: update setSelectedAccount to set the account group (#22120)
- fix: cp-7.60.0 Fix address validation for Tron (#23172)
- fix: cp-7.60.0 predict claim button style (#23153)
- fix: cp-7.60.0 automatic highest balance token in metamask pay (#23152)
- fix: cp-7.60.0 infinite loader in metamask pay (#23150)
- fix: fix unknown chain icon display issue in permissions (#21914)
- fix: cp-7.60.0 keyboard in metamask pay asset picker (#23148)
- fix: cp-7.60.0 no quotes alert message (#23120)
- fix: earn banner border and background are wrong colors (#22275)
- fix: add staked energy and staked bandwidth to nontradabletokens list cp-7.60.0 (#23128)
- fix: cp-7.60.0 patch TokenBalancesController to resolve missing aggregated balance (#23113)
- fix: cp-7.60.0 bump tron 1.10 (#23106)
- fix: add logic to compute the correct non-EVM network image source using `getNetworkImageSource` (#23089)
- fix: Implement nonEVM zero balance check to send flow before `onAmountInput` (#23037)
- fix: add badge to bridge activity list entries cp-7.60.0 (#23076)
- fix: start animate when rive onPlay ( ready ) cp-7.60.0 (#22982)
- fix: remove GNS logic from network selection and use enabled network state (#22838)
...
- fix: cp-7.60.0 bump transaction controller and transaction pay controller versions (#23029)
...
- fix: cp-7.60.0 predict withdraw with missing token (#22908)
- fix: cp-7.60.0 predict deposit layout (#22937)
- fix: increase Node.js heap size for iOS bundle generation in CI (#22971)
- fix: remove crypto compare fallback (#22772)
- fix: SL-296: handle on biometric cancel - cp-7.60.0 (#22774)
- fix: loading issues in asset details page (#22916)
...
- fix: various swaps UI discrepancies (#22717)
- fix: SL-323: login screen design fixes (#22781)
- fix: only mainnet eth should be stakable cp-7.60.0 (#22778)
- fix: scrolling issue in asset list prevents rendering bottom assets (#22792)
- fix: cp-7.60.0 hide metamask pay transaction notifications (#22794)
- fix: add resync mechanism to sync Snap accounts states with client accounts cp-7.60.0 (#22649)
- fix: fix carousel design (#22805)
- fix: cp-7.60.0 hide monad from supported networks in boost cards (#22835)
- fix: add delay before revoke tokens (#22769)
- fix: Decrease threshold before RPC failover is activated (#21432)
- fix: cp-7.60.0 bump bitcoin (#22803)
- fix: cp-7.60.0 predict confirmation design (#22745)
- fix: swaps amount truncation and received tokens action key (#22705)
- fix: useEffect infinity loop in SimpleWebview (#22534)
- fix: cp-7.59.0 Fix layout of small devices in asset amount (#22703)
- fix: Pass provider param to ChoosePassword page (#22695)
- fix: validate insufficient balance for bridges (#22481)
- fix: getGenericPassword errors (#22523)
- fix: transition to BIP-44 selectors for rewards row cp-7.59.0 (#22742)
- fix: prevent concurrency for `createAccount` for Snap account providers cp-7.59.0 (#22719)
...
- fix: Change to available fiat value text when fiat mode is enabled (#22541)
...
- fix: Exclude token transfers from marking them as a swap tx cp-7.59.0 (#22607)
- fix: pay hide testnet assets in metamask pay (#22619)
- fix: ui issues (#22605)
- fix: run discovery and alignment upon unlock for all wallets cp-7.59.0 (#22598)
...
- fix: trigger rewards animation value update on state transition cp-7.59.0 (#22351)
...
- fix: back arrow perps home should always navigate to wallet home cp-7.59.0 (#22288)
...
- fix: cp-7.59.0 hotfix-7.58.2 update close position calculation with funding fees and live data (#22229)
- fix: btc account selection during btc network change cp-7.59.0 (#22328)
- fix: re order migrations 105, 106, 107 cp-7.59.0 (#22276)
- fix: hide trending and show browser when ff is false (#22209)
- fix: cp-7.59.0 ensure that txs show on asset details page (#22285)
- fix: Update Ledger support link to the correct article (#22297)
- fix: add load tests for assets (#21912)
- fix: cp-7.59.0 Fix minimum BTC amount validation in send flow (#22245)
- fix: navigate users directly to blockchaine explorer on Solana swaps (#22048)
- fix: user interface descriepenses (#21938)
- fix: backdrop press on edit rpc bottom sheets (#22098)
- fix: 20839 fix the qr name not matched in swap page. (#22043)
...
- chore(runway): cherry-pick chore: Bypass interstitial modal for URLs that originate from within app (#23631)
...
- feat: update bottom tab button route icon label and replace it with the new one (#21665)
- feat: updated import wallet ui for Add Wallet flow in home screen (#21686)
- feat: Added temp ButtonFilter (#22089)
...
- feat: add error_type to REHYDRATION_PASSWORD_FAILED event (#21906)
- feat: updated toast ui and added description (#21942)
...
- feat: add missing dots to settings descriptions (#22053)
- feat: use new asset comparisson (#22049)
- feat: Await simulation results before disclosing gas fees (#22007)
- feat: Add MetaMetrics event for "Use different login method" click during social login rehydration (#21903)
- feat: TAT-1073 as a user i can share my pnl hero card with my contacts (#21880)
...
- feat: update bitcoin snap version (#21606)
- feat: Update text for perps "ways to earn" in Rewards (#21923)
- feat: remove pre push hook in favor of ci fix (#21904)
- feat: add Token Insights Sheet and integrate with BridgeDestTokenSelector (#21303)
- feat: Predict Analytics - Market Details, Position, and Activity Tracking (#21852)
...
- feat: add account type labels to asset details (#21849)
- feat: added RPC selector in accouts list (#21425)
- feat: align quote card design with Figma specifications (#21656)
...
- feat: remove skip bottomsheet on click of remind me later button (#21766)
- feat: add monad network (#21491)
- feat: Add NFTs full page (#21647)
- feat: add pre-push hook to prevent unformatted code from reaching remote (#21335)
- feat: Add tokens full page (#21628)
- feat: remove AccountBackupStep1 screen and modify manualbackup screens from onboarding(SRP flow) (#21510)
- feat: New Persistence System Improvements (#21225)
- feat: create feature flag for trending tokens (#21655)
- feat: speed up swap quote availability with SSE (#20909)
- feat: rewards compatible with basic functionality (#21470)
- feat: Analytics improvements and error logging examples (#21543)
- feat: update recipients UI on swaps cp-7.58.0 (#21565)
- feat: card onboarding integration (#21455)
- feat: add injective network (#21291)
- feat: add cronos network (#21200)
- feat: Add pending withdraw and deposit to Perps Activity tab (#20904)
- feat: quote card updates (#21485)
- feat: TAT-1863: as a user i want to see adl events in my trade activity page (#21442)
...
- fix: cp-7.59.0 Update minimum BTC amount (#22401)
- fix: cp-7.59.0 remove hip-3 stocks list all filter (#22462)
- fix: Close all orders/positions bottom sheet dismissal (#22216)
- fix: Android In-App Browser Crash (#22212)
- fix: adding spacing for action buttons in token details (#22206)
- fix: Deduplicate y-axis values in TradingView chart (#22201)
- fix: Updated main navigator to conditionally show rewards or settings tab (#22199)
- fix: evenly distribute price points (#22193)
- fix: getcandidatesubscriptionid silent auth if no subscription in state (#22124)
- fix: textInput focus issue (#22177)
- fix: open Polymarket terms in in-app WebView (#22139)
- fix: Perps Watchlist navigation params (#22153)
- fix: update toast background color from muted to section (#22159)
- fix: remove padding and border radius from order type bottom sheet (#22143)
- fix: update copy for perps trade action menu (#22155)
- fix: resolve TypeScript errors and add UUID v4 test coverage cp-7.58.0 (#22149)
- fix: convert quotes copy from title case to sentence case (#22146)
- fix: the network badges for the tokens are missing a border (#22147)
...
- fix: enable back navigation from activity view when navigating from perps market details (#22090)
- fix: inconsistent close button size on swaps (#21615)
- fix: flickering on dapp connection permissions page (#22051)
- fix: order type header should be sentence case (#22080)
- fix: perps withdraw page copy (#22082)
- fix: perps not reconnecting on account switch (#22068)
- fix: stop wrapping MMC messages in multichain substream (#22027)
- fix: Remove test task for android release builds (#22063)
- fix: inconsistent top padding in networks list (#22052)
- fix: cp-7.58.0 time remaining icon color (#22031)
- fix: Tron addresses not being derived for all accounts (#22050)
- fix: remove not needed workaround on e2e (#21992)
- fix: cp-7.58.0 Make burn and null address sending a blocking alert (#22047)
- fix: Implement short living ens resolver for `Name` component after send flow (#21515)
- fix: ensure that close value does not change when amount input focused (#21960)
- fix: Navigation behavior from PerpsHomeScreenSearch (#22013)
- fix: ensure that we know the different between change is 0 and change not set (#21981)
- fix: Handle deeplink if provided cp-7.58.0 (#22005)
- fix: ensure that close position screen uses correct decimals (#21982)
- fix: remove max button on Polygon native asset (#22002)
- fix: Polish perps order page colors (#21757)
- fix: tpsl for limit orders should not be auto selected in position view (#21889)
- fix: cp-7.57.2 remove currency rates multichain (#21996)
- fix: addresses styling on transaction details (#21998)
- fix: accountStatus page UI top padding (#21999)
- fix: add navigation lock on home screen buttons click (#21913)
- fix: token import flow is broken (#21911)
- fix: remove duplicate POL tokens from swap select token screen (#21924)
- fix: cp-7.59.0 explorer url for Monad (#21994)
- fix: limit price bottom sheet input decimal not showing (#21973)
- fix: cp-7.58.0 country code and keyboard aware view (#21959)
- fix: predict transaction type metrics (#21952)
- fix: fix SnapKeyring events forwarding (#21947)
- fix: cp-7.58.0 Update default blocked region (#21886)
- fix: input cursor jump sometime in ios (#21926)
- fix: update copy from `Create account` to `Add account` (#21765)
- fix: navigation when send confirmation is rejected. (#21848)
- fix: predict confirmation balance usage (#21805)
- fix: cp-7.58.0 formatTimeRemaining to correctly display time remaining (#21843)
- fix: change broken of Ledger Connection Tutorial Link. (#21659)
- fix: ui bugs (#21841)
- fix: Improve network connection banner (#21644)
- fix: fix invalid address behavior on custom token import (#21812)
- fix: fix clipped placeholder (#21774)
- fix: align balance loaders for account group and portfolio pages (#21746)
- fix: Display dynamic gas station fee (#21406)
- fix: formatting issues on main (#21752)
- fix: new homescreen formatting diffs from 21410 (#21743)
- fix: bitrise nightly flakiness (#21741)
- fix: gasless transaction submission method selection logic (#21637)
- fix: add account style on wallet details view (#21672)
- fix: ui bugs in OptinMetrics and OnboardingSuccess screen (#21551)
- fix: Input Text left align (#21250)
- fix: when clicking max token, show up to 5 decimals max (#21059)
- fix: Limit order card direction text (#21586)
- fix: font rendering issues by updating font preloader to preload all custom font files (#21199)
- fix: cp-7.58.0 close position disabled when receiveAmount <0 (#21622)
- fix: Fix flakey async unit tests in Perps domain (#21632)
- fix: cp-7.58.0 update swap asset balance on switch (#21625)
- fix: unit test flakiness (#21623)
- fix: cp-7.58.0 fix gasless transaction support logic (#21604)
- fix: redirect to blockexplorer from tx details (#21612)
- fix: Use account group address in `SnapUIAvatar` when applicable (#21607)
- fix: predict deposit token amount (#21603)
- fix: cp-7.58.0 Fix swipe gesture navigation in send flow amount page for Android (#21518)
- fix: add small margin bottom to TabBar (#21562)
- fix: Remove Ramp `SkeletonText` component from legacy confirmation (#21330)
- fix: disable toasts for all predict transactions (#21529)
- fix: lint ts issue fix (#21587)
- fix: update assets controllers (#21465)
- fix: Add shouldEnableAndroidPressIn prop to TempTouchableOpacity in Perps domain cp-7.57.1 (#21535)
- fix: Prevent relative paths in RNTar native modules (#21408)
- fix: rewards detects stale cache for opt in and link accounts (#21520)
...
- fix(card): Onboarding UI issues (#22263)
...
- feat(predict): update buy confirmation to include balance check (#21488)
- feat(card): move Card authentication verification out of CardSDK context (#21399)
- feat(predict): add slippage to orders to improve failure rate (#21443)
- feat(INFRA-3034): ci cirrus 2 (#21421)
- feat(card): Card authenticated info (#21390)
- feat(deposit): Locks user's region if they have submitted KYC (#20822)
- feat(card): display card button with geolocation and feature flag guards (#21306)
- feat(MultichainPermissionsSummary): Add disconnect button cp-7.57.0 (#21285)
- feat(predict): claim ux (#21311)
- feat(core-backend): add AccountActivity and Websocket for TokenBalancesController (#21111)
- feat(predict): Adds predict transaction activity detail view (#21293)
- feat(deposit): implement user limits fetch before proceeding with an order (#21174)
- feat(predict): deposit ux (#21198)
- feat(predict): adds transaction activity (#21148)
- feat(rewards): add new referred-related identities (#21049)
- feat(INFRA-3009): ios only in GHA (#20944)
- feat(predict): integrate fee collection via clob relayer (#21119)
- feat(predict): Predict Position Updates (#20779)
- feat(ramp): mark country as selected if state is selected (#21055)
- feat (ramps): reset input when keyboard is reopened (#20992)
- feat(card): Card API foundations and Authentication screen (#20984)
- feat(predict): trade via proxy wallet rather than eoa (#20994)
- feat(predict): Adds predict region check (#20661)
- feat(deposit): hide unsupported cryptocurrencies cp-7.57.0 (#20993)
- feat(predict): new buy/sell logic without on-chain tx (#20665)
- feat(SDKConnectV2): allow shared ws connection on `WebSocketTransport` (#20977)
- feat(perps): correct tab positions in market detail list (#20716)
- feat(predict): Predict Details Outcome Sorting / Chart Performance (#20880)
- feat(SDKConnectV2): Add "Return to App" Toasts (#20647)
- feat(perps): redirect to correct tab on order (#20449)
- feat(ramp): add activation keys to all non-prod envs (#20781)
- feat(ds): accept labelTextVariant prop (#20773)
- feat(predict): implement Safe fee authorization logic (#20704)
...
- feat(INFRA-2989): test repack iOS (#20573)
- feat(e2e): script and CI job to use AI for e2e test selection (#20372)
- feat(card): add Welcome screen and small UI improvements on CardHome (#20281)
- feat(card): change Card Home Viewed metric params from string to number (#20607)
- feat(predict): implement claim winnings (#20396)
- feat(rewards): add deeplink support for rewards (#20508)
- feat(perps): mixpanels events consolidation (#20468)
- feat(infra-2854): implement expo fingerprint caching and APK repacking for Android CI optimization (#20454)
- feat(ramp): add new CAIP-19 Ramp Intent (#20476)
- feat(SDKConnectV2): RPC Message Handling (#19823)
- feat(ramp): remove selectedChainId deps (#20206)
- feat(deposit): Replace hardcoded constants with dynamic SDK data fetching (#19402)
- feat(card): remove useSupportedTokens from deposit (#20335)
- feat(predict): initial PR (#19996)
- feat: control btc provider state via remote feature flag (#20989)
- feat: TAT-1662 trade fee does not adjust for market or limit order maker or taker (#21428)
- feat: add analytics tracking for Predict trade actions (#21216)
- feat: add BalanceEmptyState component (#20782)
- feat: l runners, cache updates (#21327)
- feat: predict confirmation metrics (#21348)
- feat: predict withdraw confirmation (#21309)
- feat: Add support for gas sponsorship (#21121)
- feat: TAT-1778 fix mm earn confirmation card account name issue (#21238)
- feat: cp-7.57.0 support external-link for notifications (#21367)
- feat: cp-7.58.0 max button in perps deposit (#21257)
- feat: card onboarding (#21206)
- feat: Add build type to version string (#21326)
- feat: Onboarding and ChoosePassword Loader update (#21281)
- feat: onboarding deeplink (#20566)
- feat: Swaps show BTC Native Segwit tag (#21228)
- feat: add action button event (#20967)
- feat: debounce metamask pay alerts (#21243)
- feat: Add trace callback in `TransactionController` constructor (#20569)
- feat: swaps recipient row (#20921)
- feat: btc swap button, BIP44 default pair (#21062)
- feat: Implement unsupported deeplink modal (#20795)
...
- feat: add gasless support via 7702 (#20233)
- feat: update onboarding screen and replace lottie with rive animations. (#19907)
- feat: show card spend events in activity tab (#21193)
- feat: add lukso network (#20846)
- feat: rewards bip-44 match designs (#21190)
- feat: enable fullscreen browser view from clone (#20956)
- feat: social login check added in description in create password. cp-7.57.0 (#21187)
- feat: retry in perps deposit transaction details (#21169)
- feat: add network logo and native logo for Plasma network (#20567)
- feat: add ways to earn sheet for referrals (#21124)
- feat: rwds-492 make level image tappable (#21136)
- feat: add how to earn for card spend (#21108)
- feat: update transaction pay bridge estimates (#21093)
- feat: cp-7.57.0 Perps 7.57.0 Mandatory UI Fixes (#21060)
- feat: bump `@metamask/multichain-api-middleware` to `1.2.0` (#20550)
- feat: rewards bip-44 (#20729)
- feat: predict claim confirmation (#21007)
- feat: cp-7.57.0 updated PerpsController to use v2 perps geo block remote flag (#21067)
- feat: 14414 Replace redux-persist with barebones JS persist system using FileSystemStorage (#17685)
- feat: update bitcoin snap version (#21031)
- feat: add activity details bottom sheet (#20988)
- feat: adapt emulator configs for cirrus runners (#21041)
- feat: Implement BTC labels in send flow (#21028)
- feat: predict deposit confirmation (#20926)
- feat: rehydration flow ios update (#20799)
- feat: swaps btc tx and history (#20863)
- feat: create `GasFeeTokenToast` component (#18611)
- feat: add labels for btc assets (#20859)
- feat: agentic unit test automation (#20837)
- feat: add deeplink decompression logic for sdk v2 payload (#20924)
- feat: social login success screen added for social login users and ios platform. (#20800)
- feat: Bitcoin+Tron BIP44 integration (#19971)
- feat: cicd migration self-hosted to cirrus (#20890)
- feat: Swaps btc network (#18851)
- feat: Feat/introduce native utils margelo (#20778)
- feat: cp-7.56.3 disable perps build flag and add belgium to build flag blocked regions fallback (#20790)
- feat: add dynamic carousel preview updates (#20712)
- feat: pending deposit withdraw (#20657)
- feat: introduce native utils margelo (#20654)
- feat: TAT-1820: Added tpsl and liquidation tags to perps fill history items (#20658)
- feat: reward points component animation (#20664)
- feat: update pk reveal with link to support page (#20693)
- feat: TAT-1491: use existing position leverage when placing an perps order (#20668)
- feat: TAT-1810 varada sync perps misc UI fixes (#20660)
- feat: NFT Grid View (#20150)
- feat: add pnl for perps points events (#20638)
- feat: MM SDK, add option to hide return modal (#20524)
- feat: use `wallet.status` (#20639)
- feat: PerpsTransactionsSkeleton (#20666)
- feat: add paymentRequestEnabled to webview (#18845)
- feat: Swaps consume bip44 default pairs from LD (#20179)
- feat: Perps max order error messaging (#20559)
- feat: run e2e builds on build.sh changes (#20629)
- feat: use bottom sheet navigation for fiat selection modal (#20600)
- feat: Add empty state to market list when no results found after search (#20601)
- feat: update network selector 'all networks' copy to 'popular networks' (#20570)
- feat: Metametrics UI changes (#20104)
- feat: remove selected address dependency (#20544)
...
- feat: Improve Earn and Perps remote feature flags (#20486)
- feat: Add rewards to Perps close position flow (#20355)
- feat: add network connection banner with direct edit functionality (#19202)
- feat: update NFT tab empty state (#20270)
- feat: update Perps tab empty state (#20196)
- feat: consume quoteBpsFee for dynamic fee disclaimer (#20388)
- feat: import SRP design changes. (#20101)
- feat: add multichain account syncing E2Es (#20315)
- feat: update DeFi tab empty state (#20279)
- feat: add a "Way to Earn" for loyalty bonus (#20300)
- feat: cp-7.56.0 TAT-1709: updated copy and styling for perps fullscreen announcement modal (#20276)
- feat: network manager e2e tests (#19847)
- feat: integrating `SelectedGasFeeToken` in the transaction confirmation (STX) (#18450)
...
- feat: bump solana snap (#20265)
- feat: amount page related improvements in new send flow (#20119)
- feat: add megaeth mainnet support (#20056)
- feat: add HEMI network (#20111)
- feat: cp-7.56.0 TAT-1486 update tutorial animations (#20207)
- feat: cp-7.56.0 TAT-1767: update perps tpsl bottom sheet copy button placement and behaviour (#20163)
- feat: remove password strength check (#20100)
- feat: add UI loading states for backup & sync (#20043)
- feat: enhance wallet alignment process in MultichainAccountsIntroModal (#19970)
- feat: Display account group names in Snaps components (#20031)
- feat: respect basic functionality state when disabling swaps buttons (#20102)
- feat: add no-fee badge to mUSD token row (#20022)
- feat: cp-7.56.0 TAT-1749: Added missing event properties when placing order and closing position (#20062)
- feat: cp-7.55.0 bump solana snap to 2.3.10 (#20046)
- feat: claim reward flow (#19954)
- feat: display account names for Loyalty and Sign up bonuses (#19969)
- feat: add tab empty state UI components (#19991)
- feat: Add fast coverage analysis system (#19962)
- feat: Add tracking events for account existence pages (#19645)
- feat: Prevent adding multiple txs in the BIP-44 send flow fix (#19859)
- feat: Unified activity list for BIP-44 (#19590)
- feat: add metadata properties to `RewardsController` (#19782)
- feat: update AccountCell styles and add placeholder color to search input (#19666)
- feat: add skeleton loaders to perps deposit (#19741)
- feat: add new trade menu (#19459)
- feat: display active seasonal boosts (#19562)
- feat: replace account syncing with multichain account syncing (#19246)
- feat: add Rewards 'Ways to Earn' section (#19606)
- feat: add xdc network (#19524)
- feat: choose password design updates (#19758)
- feat: rename Orders to Transfers cp-7.56.0 (#19754)
- feat: faster non env address check (#19788)
- feat: change Maskicons to Polycons for users cp-7.56.0 (#19769)
- feat: swaps rewards rive icon (#19634)
- feat: Biometrics lock fix (#19316)
...
- chore: Revert "fix: conditionally apply onPress for Accessibility mode for Android TouchableOpacity" (#21342)
- chore: Alphabetize list of controllers in `state` getter (#21480)
- chore: refactor homepage spacing to use flexbox layout with gap (#21427)
- chore: homogenize experience when adding NFT and adding token (#21259)
- chore: Bump Snaps packages (#21415)
- chore: Rename FoxRiveLoaderAnimation mock (#21431)
- chore: adds TRX Staking feature flag (#21458)
- chore: update to yarn v4 (#21207)
- chore: Remove `RewardsController` constructor side-effects (#21437)
- chore: Swaps design adjustments (#21321)
- chore(predict): adds initial logic to handle a resolved market (#21382)
- chore: update buffer polyfills - Margelo (#21386)
- chore: sync stable to main for version 7.58.0 (#21001)
- chore: format (#21422)
- chore: Update `build-utils` (#21414)
- chore: bump Tron snap version to 1.5.0 (#21358)
- chore: fix e2e test reports (#21354)
- chore: New Crowdin translations by Github Action cp-7.57.0 (#21379)
- chore(predict): refactor predict market list (#21378)
- chore: Update `eth-json-rpc-middleware` from v19 to v21 (#21366)
- chore(predict): feed / search improvements (#21353)
- chore(card): add new Baanx API key environment variables on build.sh (#21317)
- chore(predict): disable slippage (for now) (#21322)
- chore: update animations for perps (#21241)
- chore: clean e2e stuff (#21266)
- chore: remove self hosted runners (#21289)
- chore: useBridgeQuoteEvents hook (#21249)
- chore: update tron snap to latest version (#21265)
- chore: bump `@metamask/remote-feature-flag-controller` to `^1.9.0` (#21194)
- chore: add env exp to profiler (#21219)
- chore: Update activity list codeowners (#21052)
- chore: clean e2e stuff (#21196)
- chore: Ignore warning log of keychain (#21166)
- chore: Bump bridge and bridge status controller to v50.0.0 (#21137)
- chore: update js example (#21164)
- chore: e2e cleaning (#21143)
- chore: local flask builds (#21154)
- chore: update rewards animation (#21153)
- chore: integrate delegation-controller (#19964)
- chore: Cleanup remove old collectible list (#21133)
- chore: clean e2e stuff (#21081)
- chore: cp-7.57.0 improve performance on accounts list (#21098)
- chore: cp-7.57.0 chain filtering should not affect account picker balances (#21122)
- chore: add `@metamask/wallet-integrations` team as codeowners for `BackgroundBridge/` (#21117)
- chore: run AI analysis on all PRs (#21053)
- chore: split new network manager tests (#21112)
- chore: Update keychain to v^9 (#20625)
- chore: Bump `@metamask/ens-resolver-snap` to `1.0.0` (#21047)
- chore(SDKConnectV2): Update package (#21087)
- chore: add environment to the state logs (#20783)
- chore: cicd - revert to lg runners (#21009)
- chore(perps): remove mistakenly pushed task file (#20919)
- chore: Added font preloader cp-7.57.0 (#20331)
- chore: Update perps related .js.env build flags to align with main-rc (#20913)
- chore: disable RC auto generation (#20896)
- chore: cp-7.57.0 add padding between 'Your balance' and action buttons on token detail page (#20843)
- chore: bump `@metamask/smart-transactions-controller` to `^19.2.0` (#20152)
- chore: Bump Snaps packages (#20834)
- chore: New Crowdin translations by Github Action (#20670)
- chore: Bump main version to 7.58.0 (#20786)
- chore: add deprecation notices to component-library components with design-system replacements (#20760)
- chore: move appwright to dev dependency (#20354)
- chore: update `CODEOWNERS.md` -> `@metamask/sdk-devs` to `@metamask/wallet-integrations` (#20745)
- chore: fix recently introduced lint issue (#20776)
- chore: account for rc builds on browserstack (#20753)
- chore: Update Earn controller to resolve peer dependency warnings (#20044)
- chore: Bump target SDK to 35 (#20461)
- chore: Bump Snaps packages (#20684)
- chore(test): add option to skip react native reload in test cleanup (#20527)
- chore: set BIP-44 as default cp-13.5.0 (#20250)
- chore: E2E docu updated (#20709)
...
- chore: bump `@metamask/multichain-account-service` to `^1.5.0` (#20687)
- chore: design fixes in swaps (#20662)
- chore: Make remote image use expo-image (#20606)
- chore: upcoming rewards to locked rewards (#20618)
- chore: use sids from ois endpoint (#20628)
- chore: Standardize network connection banner type/metrics (#20602)
- chore: dedicated github runners for android e2e (#20463)
- chore: rewards bottom sheet modal description bodyMd (#20619)
- chore: remove network restrictions from swaps buttons (#20529)
- chore: Prioritize target and environment values from js env for build.sh (#20614)
- chore: bump `@metamask/multichain-account-service` to `^1.4.0` (#20572)
- chore: removes null snapshot (#19269)
- chore: tweaks to settings view for multi subscription case (#20575)
- chore: adds Tron snap support (#18303)
- chore: switch to mobile optin endpoint (#20554)
- chore: add segment error tracking to vault issues (#19425)
- chore: Make close position values reactive to price changes (#20546)
- chore: Add missing locale string (#20540)
- chore: modals for unlinked accounts in rewards dashboard (#20511)
- chore: bump `@metamask/profile-sync-controller` to `^25.1.0` (#20513)
- chore: update old identity code ownership (#20502)
- chore: refactor recipient validation in new send implementation (#20229)
- chore: scale e2e-tests to large runners (#20391)
- chore: add cursoragent to CLA exception (#20475)
- chore: enhanced tab to lazy load and scrollable cp-7.56.0 (#19990)
- chore: stable main sync 7.57.0 (#20334)
- chore: Update .js.env.example with dev and qa values (#20485)
- chore: Remove unused patch scripts and script that was using patch package (#20443)
- chore: rewards error banner match designs & onboarding modal for geo/… (#20452)
- chore: Replace local with dev environment and point to correct environments (#18064)
- chore: update network icons (#20438)
- chore: update network names (#19745)
- chore: bump multichain accounts deps (#20255)
- chore: add e2e smoke release label to release PR when created (#18010)
- chore: replace react native tcp by react-native-tcp-socket package (#20384)
- chore: remove nft controller patch (#20312)
- chore: reduce mobile login (#20400)
- chore: improved rewards ui for error and loading (#20389)
- chore: solana snap update (#20382)
- chore(deps): bump `@metamask/eth-hd-keyring` to 13.0.0 (#20325)
- chore: cleanup code to use assets returned by selector selectAssetsBySelectedAccountGroup (#20316)
- chore: error timeout feedback (#20329)
...
- fix(predict): small ui/ux fixes (#21381)
- fix(predict): change order toasts and show claim amount correctly (#21356)
- fix(predict): predict resolved positions component (#21301)
- fix(SDKConnectV2): Fix storage regression (#21283)
- fix(deposit): remove unresponsive period from keypad cp-7.57.0 (#21233)
- fix(perps): adjust decimals logic accross perps codebase (#20971)
- fix(perps): optimize connection speed and consolidate architecture cp-7.57.0 (#20946)
- fix(deposit): payment method duration badge (#21140)
- fix(SDKConnectV2): Fixed race condition and expiry bug (#20990)
- fix(action): require changelog entry even if the PR description doesn't match template (#21021)
- fix(pr template): Use latest version of release pr template (#21019)
- fix(perps): remove loading delay from tutorial screen (#20918)
- fix(predict): market details tabs (#20970)
- fix(perps): cp-7.57.0 missing trace attachment in sentry measurements (#20817)
- fix(predict): safe nonce when contract is not deployed (#20963)
- fix(deposit): get started mUSD cp-7.57.0 (#20915)
- fix(ramp): prevent 'undefined' flash in sell amount input during load cp-7.57.0 (#20881)
- fix(deposit): error modal cp-7.57.0 (#20772)
- fix(deposit): region selector modal param cp-7.57.0 (#20793)
- fix(release pr approval): add more explicit name for the job (#20856)
- fix(SDKConnectV2): Storage will automatically delete expired connections (#20565)
- fix(perps): cp-7.57.0 perps reward discount not applied correctly (#20738)
- fix(SDKConnectV2): Fix in-app QR Code scanner to handle SDK V2 deeplinks (#20691)
- fix(SDKConnectV2): UI not showing the right dapp name/url/icon (#20623)
- fix(SDKConnectV2): SDKDisconnectModal `toHex` issue (#20680)
- fix(sign off list): remove legacy list of teams (#20720)
...
- fix(ramp): payment method selector height (#20651)
- fix(perps): invalid reward icon in perps market list (#20444)
- fix(account-tree-controller): re-use computed names for groups (#20630)
- fix(perps): Update fee display text color to alternative in Perps trade screen (#20445)
- fix(perps): invalid candle interval button styles (#20447)
- fix(rewards): added relevant margins for alignment on rewards dashboard (#20586)
- fix(multichain-accounts): use proper `selectedAccountGroup` (instead of `selectedInternalAccount`) (#20517)
- fix(SnapKeyring): do not use approval flow if confirmation is skipped (#20456)
- fix(ramp): display all orders from account (#20432)
- fix(perps): cp-7.56.0 events name in snake_case (#20455)
- fix(fitness function): add more explicit name for the job (#20395)
- fix(rewards): fixed toast postion and network list spacing issues (#20352)
- fix(ramp): safe area view (#20322)
- fix(rewards): fixed alignment and referral code validation logic (#20305)
- fix(ramp): order list for items with wrong network cp-7.56.0 (#20251)
- fix(ramp): use account group name (#20319)
- refactor: use `@metamask/eth-qr-keyring` (#17157)
- test: improve test execution consistency (#20266)
- test: adds unit tests to run-e2e-tags-gha.mjs (#20306)
- test: Fixed test `e2e/specs/wallet/send-ERC-token.spec.js` (#20261)
- chore: rwds-390 wrong level image (#20303)
- test: disables flaky test (#19974)
- test: refactors the e2e sh script to mjs for flakiness detection (#19838)
- chore: Add Solana devnet support (#20155)
- chore: rwds-468 fix stale balance (#20243)
- chore: RWDS string changes based on testing / user feedback (#20288)
- refactor: refactor BIP-44 feature flag (#20109)
- chore(ramps): migrate to react-native-safearea-context for android 35 (#19136)
- feat(card): use supported list of deposit tokens on Card home (#19949)
- feat(card): add card home viewed metric (#20041)
- chore: update tab empty state button to secondary variant (#20262)
- refactor: remove iconUrl from event asset and update ActivityEventRow to use network images (#20244)
- fix(perps): cp-7.56.0 validate metametrics events (#20227)
- chore: cicd cleanup sha-pinning (#20242)
- feat(perps): cp-7.56.0 market balance actions component (#20107)
- test: update account name change flow in multichain accounts tests (#20216)
- test: BIP-44 Account wallet details e2e tests (#20213)
- refactor: portfolio value format (#20195)
- feat(INFRA-2969): small runners (#20224)
- chore: rwds-475 (#20235)
- fix(rewards): removed prefix text on referral link (#20232)
- feat(MultichainAccountSelectorList): do not show wallet footer for hardware wallets and imported accounts (#20129)
- chore: rewards fix multi account subscription issues (#20162)
- chore(deps): bump `@metamask/solana-wallet-snap` to 2.3.10 (#20220)
- chore: Change one word in en.json for RWDS (#20215)
- refactor(ramps): update aggregator token modal to a bottom sheet (#19185)
- test: add multichainAccountsIntroModalSeen flag to default FixtureBuilder (#20210)
- ci: fix triggering performance e2e jobs (#20208)
- New Crowdin translations by Github Action (#20176)
- fix(perps): cp-7.56.0 fee calculation discrepency between ui and order execution (#20184)
- chore: send up usd price per token with rewards API request in Swaps (#19768)
- chore(CODEOWNERS): update wallet-ux to be mobile-core-ux (#19851)
- chore: integrate @metamask/gator-permissions-controller (#20006)
- ci: clean up performance e2e workflow (#20192)
- chore: bump SelectedNetworkController package (#19235)
- chore: rewards rwds 358 (#20140)
- refactor(perps): change reward controller cache duration to 5min (#20134)
- fix(perps): cp-7.56.0 invalid margin computation in close position screen (#20172)
- fix(perps): cp-7.56.0 inconsistent style in fee display (#20174)
- chore: updated fox icon for rewards (#20175)
- chore: Update `multichain-network-controller` (#19978)
- New Crowdin translations by Github Action (#19898)
- chore: bump account-tree-controller to ^0.18.1 (#20151)
- chore: change reward tab icon (#20118)
- style: Overview tab UI tweaks (#20130)
- chore: Collect metrics for unavailable/degraded custom RPC (#20030)
- fix(AccountGroupDetails): hide private keys button for hardware wallets (#20045)
- chore: reward auth compatible with solana (#20052)
- Fix: bip 44 bugs (#20013)
- chore: sentry rewrite breadcrumb perf (#20012)
- chore: Updated slide animation for settings to slide in from the right (#19989)
- chore: bump multichain accounts deps (#20040)
- chore: Add disable Husky instructions to readme (#20003)
- chore: Update `@metamask/swaps-controller` to v14 (#19935)
- INFRA-2911: Hardcoded parameters values for create release pull request (#19353)
- refactor(AccountCell): update navigation to account group details (#20035)
- chore: use exported constants for user storage paths (#20024)
- chore: update default swap pairs for Ethereum and Linea to mUSD (#20018)
- feat(SDKConnectV2): Connection Resumption & App State Handling (#19505)
- chore: update BIP-44 feature flag override to disable the feature (#19886)
- test: Account list - import SRP (#19955)
- test: fix performance e2e: ios onboarding (#20009)
- test: Fixed onramp tests (#20005)
- test: perps perf e2e onboarding (#19966)
- feat(perps): integrate MetaMask Points rewards and fee discounts for Perps trading (#19808)
- chore: upgrade assets controllers to v75.2.0 (#19742)
- test: add profiling data to performance test reports (#19875)
- chore: Fix rive docs (#19878)
- chore: Add additional safeguard measures to further prevent unnecessary truncations cp-7.56.0 (#19857)
- chore: bump multichain accounts deps (#19932)
- ci: remove clean runner step for iOS E2E tests (#19940)
- test: Delete and import account with PK (#19795)
- chore: remove low prio (#19379)
- chore: Update `ppom-validator` (#19340)
- chore: Update `eth-snap-keyring` and `keyring-api` (#19917)
- ci: remove nick-retries from E2E run step (#19909)
- Rwds 270 level image name points threshold should come from rewards backend (#19728)
- New Crowdin translations by Github Action (#19796)
- test: update mock profile-sync fixtures (#19879)
- chore: draft workflow for rc builds in rwy (#19472)
- chore: cp-7.56.0 update perps build flags ahead of launch (#19853)
- chore: Add new metadata to `PerpsController` (#19827)
- ci: only run performance job on release commit messages (#19840)
- chore: faster network icon (#19789)
- RWDS-289 new header key for rewards auth (#19767)
- chore: adds Bitcoin testnet feature flag (#19752)
- Feat/default-biometric-choice (#19732)
- INFRA-2919: Fix running infinite bitrise workflows on autotriggers (#19771)
- chore: sync stable to main for version 7.54.2 (#19639)
- New Crowdin translations by Github Action (#18904)
- feat(deposit): adds ETH to list of deposit tokens (#19757)
- chore: Bump main version to 7.57.0 (#19762)
- chore: migrate to new SafeAreaView (#19747)
- fix: ShareAddressQR displays correct address for non-EVM assets (#21449)
- fix: Added TempTouchableOpacity to conditionally simulate tap behavior for android cp-7.57.0 (#21426)
- fix: Check case sensitive `Content-Length` header (#21475)
- fix: incorrect network when adding token (#21461)
- fix: cp-7.57.0 update destination chain balances after bridge transaction (#21430)
- fix: invalid balance passed to parseUnits (#21464)
- fix: cp-7.57.0 caip-issue causing failure to show points estimation (#21462)
- fix: update default swap destination for Sei (#20731)
- fix: improve perps tutorial carousel layout for longer translations (#21295)
- fix: exit useLatestBalance early on undefined chain (#21407)
- fix: cp-7.57.0 deeplink for rewards populates referral code (#21403)
- fix: Navigation title misalignment on Android (#21280)
- fix: formatting diffs 20904 (#21315)
- fix: cp-7.57.0 TAT-1888 fix perps controller geo block remote feature flag race condition (#21385)
- fix: patch bridge status controller 50 (#21374)
- fix: cp-7.58.0 metamask pay transaction failures (#21287)
- fix: update package to fix staking balance issue (#21222)
- fix: cp-7.58: fix amount/decimals in rewards activity (#21334)
- fix: Swaps incorrect token icon (#21304)
- fix: Fix stale block tracker cache bug (#21288)
- fix: rewards third iteration for bip-44 (#21286)
- fix: reduce number of notification details (#21279)
- fix: metamask pay metrics in finalized events (#21237)
- fix: fix displayed warning for network (#21091)
- fix: NFT image does not load when importing nft after app restart (#21267)
- fix: chart not displaying sometimes on android (#21211)
...
- fix: now consider non-EVM account names for group names - cp-7.57.0 (#21189)
- fix: select first gas fee token automatically (#18753)
- fix: Broken deposit monitor toasts cp-7.57.0 (#21165)
- fix: BackgroundBridge should not check the request origin when the connection is from WC or SDK (#21118)
- fix: cp-7.57.0 Initialize feature flags from state (#21168)
- fix: cp-7.57.0 slow reward signing, update sol snap (#21202)
- fix: conditionally apply onPress for Accessibility mode for Android TouchableOpacity cp-7.57.0 (#21146)
- fix: prevent swaps screen from briefly flashing bridgeview when loading (#21116)
- fix: multichain account network permissions switch cp-7.57.0 (#21043)
- fix: adds solana and btc account balances back to ramp quote page (#21104)
- fix: polygon in metamask pay (#21191)
- fix: cp-7.57.0 accounts list not scrollable on android (#21180)
- fix: Update text size in Tag to match designs (#20999)
- fix: update padding and icon sizing on the homepage (#21171)
- fix: cp-7.57.0 hide 0 balances account picker (#21139)
- fix: order size and margin displayed on trade screen differ from execution for small amounts (#20576)
- fix: rewards bip-44 modify opt out event emit (#21123)
...
- fix: BackgroundBridge onMessage origin check (#20987)
- fix: transaction pay loading (#21090)
- fix: update font size in RPC flow (#21056)
- fix: bump `@metamask/account-tree-controller` to `v1.4.2` cp-7.57.0 (#21046)
- fix: asset details showing -100% token price in Swaps UI (#21026)
- fix: consume send bundle feature flag to determine alternateGasFee support (#19899)
- fix: cp-7.67.0 improve tabs click and move polling provider to parent element (#20898)
- fix: cp-7.57.0 fix api fetcher balances (#21014)
- fix: Prevent throwing exception when save is clicked on cancel and speedup with a UI error (#21038)
- fix: Fix fallback of gas market estimates on send flow (#21037)
- fix: Adding missing metrics property is_smart_transaction for smart transactions (#21027)
- fix: Fix issue 20841 QR confirmation screen appearing after re-open MM. Unable to close. cp-13.5.0 (#21024)
- fix: cp-7.57.0 fix padding after tabslist change (#20986)
- fix: lock time when skipping onboarding and test coverage (#20865)
- fix: seedless-rehydrate and sync issue (#20829)
- fix: deprecate legacy swaps (#20947)
- fix: Fix name of Snap keyring builder messenger (#20981)
- fix: cp-7.57.0 wallet initiated Send transactions are validated by security alerts API on mobile (#20979)
- fix: account backup screen ui in SRP flow. (#20972)
- fix: network `gasFeeTokenIcon` and display native amount as fallback cp-7.57.0 (#20931)
- fix: Addressing designs feedbacks on send amount page (#20973)
- fix: Updated rewards page spacing to allow active boosts's carousel to go to edge of screen cp-7.57.0 (#20939)
- fix: cp-7.57.0 Minimal pending deposit UI (#20910)
- fix: cp-7.57.0 handle URL-encoded characters in browser URL processing to fix home button navigation (#20883)
- fix: cp-7.57.0 no duplicate season status calls on account add (#20942)
- fix: webview protocols alert dialogs (#19383)
- fix: cp-7.57.0 Fix insufficient alert on send max (#20933)
- fix: prevent duplicate switch requests in account network row modal (#20633)
- fix: skip specific regression tests for android and skip ios suite in ci (#20920)
- fix: Streamlined tabs underline bar logic and added padding to tabs content for better scroll experience cp-7.57.0 (#20740)
- fix: cp-7.57.0 Change rewards feature flag (#20886)
- fix: cp-7.57.0 bump assets controllers to v78.0.1 (#20801)
- fix: fix push events to contain better mixpanel data querying (#20855)
- fix: cp-7.57.0 update rewards event type for take profit and stop loss (#20869)
- fix: cp-7.57.0 disable swaps for unsupported chains (#20838)
- fix: Fix simulation total value to be settled in `properties` instead of `sensitiveProperties` (#20840)
- fix: cp-7.57.0 Fix new send flow metrics (#20826)
- fix: use old account indicator instead of checkmark cp-7.57.0 (#20643)
- fix: adds backup button click handler and navigates to backup flow (#20832)
- fix: cp-7.57.0 rewards gtm spawn modal different session than bip44 (#20807)
- fix: cp-7.57.0 remove rewards auth grace concept (#20804)
- fix: cp-7.57.0 prevent duplicate points events api calls (#20802)
- fix: cp-7.57.0 UI tour images are narrow small (#20775)
- fix: align connection logic with extension cp-7.57.0 (#20578)
- fix: cp-7.57.0 MM Poly font on iOS & onboarding title cropped (#20736)
- fix: cp-7.56.2 add contentful max version number segmentation (#20706)
- fix: cp-7.57.0 filter chainIds for staked balances (#20755)
- fix: cp-7.57.0 rive asset state machine name (#20758)
- fix: wallet-connect qrcode redirect (#20636)
- fix: cp-7.56.2 force rendering on token list when order changes (#20697)
- fix: tab bar alignment (#20746)
- fix: cp-7.57.0 side effect opt in default account in srp (#20747)
- fix: cp-7.57.0 handle session expiration for rewards module (#20690)
- fix: cp-7.57.0 make sure rewards session token exists when importing SRP that h… (#20742)
- fix: Nft grid refresh control not working correctly (#20726)
- fix: cp-7.56.2 remove unintended metrics from transaction finalised event (#20700)
- fix: add a loading state when loading prices instead of 0 00 incl in perp asset page (#20708)
- fix: change backup text color in account details (#20703)
- fix: env vars reading utils (#20701)
- fix: add missing Rewards translations (#20696)
- fix: design inconsistencies for wallet details (#20669)
- fix: standardize button icon sizes to md (default) for bottomsheets, navigation and modals (#20547)
- fix: remove 'Your QR code' tab from QR scanner (#20583)
- fix: clear destAddress when destAccountSelector unmounts (#20686)
- fix: only enable solana devnet in flask (#20682)
- fix: recipient validation for internal accounts (#20674)
- fix: multichain accounts intro modal animation and copy (#20672)
- fix: show toast when switching account (#20394)
- fix: improve performance of MultichainAccountSelectorList with pre-rendering and scroll optimizations (#20577)
- fix: resolve prop override in PerpsEmptyState component (#20534)
- fix: custom simulator name with space failing to start (#20646)
- fix: Returned Scrollview to Perps and Defi tab cp-7.56.1 (#20642)
- fix: Update styling of tag component for Bitcoin support (#20585)
- fix: remove ">" on ways to earn and align fonts (#20632)
- fix: reduce re-renders for token items (#20620)
- fix: Rewards activity tab timestamp truncated (#20581)
- fix: Perps token logo rendering (#20596)
- fix: Temp Revert page-level scroll for Wallet (#20579)
- fix: style design inconsistencies for account group details (#20488)
- fix: hardware back button on android goes back too far in smart account view (#20479)
- fix: cp-7.56.1 dApp swaps failure on non-permitted network (#20551)
- fix: add wallet toast message (#20568)
- fix: Autodetect NFTs does not detect NFTs on Linea and Ethereum Mainnet (#20504)
- fix: prevent crash opening swaps for tokens with large balances (#20523)
- fix: handle invalid timestamp (#20484)
- fix: Improvements in ens resolution code in new send implmentation (#20532)
- fix: Clean up watcher commands and make it platform agnostic (#20560)
- fix: Dont error when MM_SENTRY_AUTH_TOKEN is absent (#20556)
- fix: resolve account balance display issue in account lists (#20417)
- fix: properly show 0 balance tokens as 0 (#20535)
- fix: if network exist do not switch (#20245)
- fix: not being able to see the apply button in swaps source network picker (#20531)
- fix: bug where 501 would be shown under sol native asset (#20537)
- fix: tab empty state design polish (#20483)
- fix: guard setBasicFunctionality calls with state 2 feature flag (#19939)
- fix: Update tooltip header for petname (#20503)
- fix: view on blockexplorer links and names for supported networks (#20506)
- fix: fixed minus symbol position in fiat formatting (#20514)
- fix: not using the correct solana balances (#20349)
- fix: missing transactions in activity after perps deposit (#20507)
- fix: App State Handling (SDKConnectV2) - Permission Revoke (#20509)
- fix: fixed leverage typo in perps tutorial (#20496)
- fix: cp-7.56.0 keyboard pushing swaps search bar off screen (#20492)
- fix: resolve Segment analytics initialization failure on app restart (#20465)
- fix: (Browser) URL with RTL chars validation (#20280)
- fix: cp-7.56.0 use BottomSheet and set a height for it (#20406)
- fix: improved UI/UX of SmartAccountNetworkList (#20401)
- fix: use correct account name variable for QR code shader header (#20433)
- fix: keyboard overlapping search results on iOS (#20428)
- fix: fix network selector dapp (#20448)
- fix: cp-7.46.0 New Crowdin translations by Github Action (#20212)
- fix: unable to perform solana transaction if an ethereum provider was connected first (#20249)
- fix: performance tests (#20439)
- fix: debounce swap token selection for slower devices (#20404)
- fix: Bump Sentry to 6.15.0 to fix Promise.allSettled crash on local iOS (#20423)
- fix: browserstack build names for performance tests (#20434)
- fix: check workflow (#20425)
- fix: regression with new TradeWalletActions not linking to perps tutorial (#20341)
- fix: remove redundant marketing consent bottom sheet for social login user cp-7.56.0 (#20379)
- fix: Missing Perps translations (#20278)
- fix: cp-7.56.0 TAT-1789 fix PerpsBottomSheetTooltip freezing on android (#20398)
- fix: Fix Solana send to redirect activity list (#20387)
- fix: skip `multipleApprovalsAlerts` when simulations is not supported (#19435)
- fix: existing EVM permissions removed when a Solana Wallet Standard connection is rejected (#20309)
- fix: Update Petnames to show Account and Wallet name with BIP44 flag (#19973)
- fix: handle transaction failures in confirmation (#20314)
- fix: SDK "return to app" modal (#19921)
- fix: cp-7.56.0 Misc Fixes for Perps GTM fullscreen modal and Tutorial flow (#20348)
- fix: handle rewards url per env (#20304)
- fix: cp-7.46.0 wrap useInitialSource/DestToken in useEffect (#20336)
- fix: cp-7.56.0 allow tpsl form to enter profit in sl and loss in tp for existing position (#20317)
- fix: bold account list item text (#20264)
- fix: reset onboarding step if user exits (#20332)
- fix: use rewards toast (#20328)
- fix: add metametrics section in security settings for social login users cp-7.56.0 (#20298)
- fix: restore unified activity footers (#20260)
- fix: e2e setup and teardown settings (#20299)
- fix: cp-7.56.0 speed up source token picker network change, skip token fetching (#20211)
- fix: seedless controller lock (#20116)
- fix: remove src asset when gotoswap (#20275)
- fix: correct feature flag name in useAccountInfo test (#20282)
- fix: resolve AddressList header flickering issue (#20197)
- fix: cp-7.56.0 fix hidden pooled-staking learn more button in historic apy bottom sheet (#20272)
- fix: recalculate trade button cutout on font scale change (#20256)
- fix: ways to earn to perps route (#20263)
- fix: rewards input and small screen issues (#20252)
...
- fix: account switching in account permission list (#19730)
- fix: cancelling send request from confirmation footer should go back to home page (#20239)
- fix: Truncate URL appropriately on iOS (#17924)
- fix: social login disable metametric option cp-7.56.0 (#20221)
- fix: solana account switching (#20131)
- fix: Display account & wallet name in the `AccountNetworkInfo` component (#19975)
- fix: add `autoCapitalize=none` to password input fields in PrivateKeyList and RevealPrivateCredential components (#20230)
- fix: account maximum length name on account list (#20226)
- fix: cp-7.56.0 metamask pay alert button flicker (#20103)
- fix: wait for snap keyring before account sync (#20219)
- fix: cp-7.56.0 missing bridge symbols in transaction details (#20067)
- fix: fix price chart for solana (#20145)
- fix: cp-7.56.0 Perps socket persisting on account switch v5 (#20060)
- fix: show check for selected account group in dapp connection (#20137)
- fix: update login log (#19923)
- fix: rewards onboarding for small screens (#20204)
- fix: cp-7.56.0 order initial amount is 0 when it should be 10 (#20203)
- fix: navigate to address list on receive button (#20015)
- fix: cp-7.56.0 prevent destination account picker infinite reload (#20180)
- fix: cp-7.56.0 add multichain prices polling (#20149)
- fix: disable skeleton animation in unit tests (#20127)
- fix: remove account permissions before account removal (#20042)
- fix: account avatars in site cell (#19972)
- fix: Send flow design defects (#20142)
- fix: cp-7.56.0 force USD currency in perps deposit (#19952)
- fix: onboarding metametric with social login cp-7.56.0 (#20027)
- fix: Correct one locale entry for rwds (#20110)
- fix: activity decimal places (#20108)
- fix: swap bottom sheet triggers old swap UI (#20106)
- fix: rewards stack navigation (#20105)
- fix: UI fixes to onboarding flow (#20066)
- fix: removed all remaining centra-no1 references cp-7.56.0 (#19855)
- fix: fixes multiple UI/UX Rewards elements (#20055)
- fix: disabled solana filter (#20053)
- fix: cp-7.56.0 readd steps for workflow dispatch (#20051)
- fix: cp-7.56.0 make performance e2e account for [skip-ci] (#19953)
- fix: changing rewards default backend host (#19931)
- fix: remove old and unused UserStorageController state properties (#20026)
- fix: cp-7.56.0 hide networks with no tokens in metamask pay (#19908)
- fix: non blocking Account sync & discovery operations when importing a new SRP (#19985)
- fix: isolate ActiveBoosts horizontal swipes from TabsList (#19997)
- fix: cp-7.56.0 TAT-1761 no toast displayed for deposit transaction failure (#20008)
- fix: cp-7.56.0 TAT-1703 Perps pre-release toast fixes (#19934)
- fix: cp-7.55.0 Update hardcoded staking duration (#19992)
- fix: Account group details Mask icon to match the account list (#19988)
- fix: cp-7.56.0 tpsl internationalization keyboard (#19764)
- fix: navigate to appropriate token modal in edge cases (#19942)
- fix: show default selected account first in dapp connection (#19731)
- fix: should not switch network after swap transaction (#19410)
- fix: move multichain accounts discovery to when onboarding is complete (#19920)
- fix: cp-7.56.0 handle small fiat values in metamask pay (#19906)
- fix: replace Pressable with ButtonBase to resolve Android touch issue cp-7.55.0 (#19948)
- fix: show checkbox with multiselect (#19665)
- fix: Initial scroll to selected account for MultichainAccountSelectorList (#19880)
- fix: prevents showing multichain accounts new feature modal on fresh install (#19933)
- fix: add ellipsis to long account names in AccountGroupDetails (#19915)
- fix: Cannot read property 'requiredScopes' of undefined cp-7.56.0 (#19837)
- fix: cp-7.56.0 Persisting toasts (#19784)
- fix: BIP44 asset overview balance issues (#19912)
- fix: change how icon scam warning is detected (#19820)
- fix: cp-7.56.0 notification item/modal style fixes (#19863)
- fix: Segment analytics events not being sent to servers (#19856)
- fix: `usePerpsLiveAccount` stream to update balance via websocket (#19883)
- fix: cp-7.56.0 add fiat formatting utility settings for liquidation price (#19861)
- fix: cp-7.56.0 if no position tpsl but >1 general tpsl show count (#19783)
- fix: cp-7.56.0 fix network enablement (#19734)
- fix: remove top edge for new SafeAreaView (#19798)
- fix: Scroll to the selected account index automatically in MultichainAccountSelectorList (#19766)
- fix: cp-7.56.0 fix selector for performance test (#19778)
- fix: cp-7.56.0 fix network list crashing due to bad evm logic (#19759)
...
- fix: Returned Scrollview to Perps and Defi tab cp-7.56.1 (#20650)
...
- fix: cp-7.56.1 Temp Revert page-level scroll for Wallet (#20616, #20579)
...
- feat(ramp): remove network filtering from the token list (#19476)
- feat(card): add badge on Card button (#19411)
- feat(perps): improve Android reconnection and centralize grace period management (#19635)
- feat(accounts): add end-to-end tests for changing account names in multichain setup (#19283)
- feat(deposit): deeplink initial support (#19404)
- feat(perps): add deeplink for handling the market list screen (#19525)
- feat(perps): implement RoE-based TPSL calculations with leverage awareness (#19171)
- feat(perps): screen cleanup and connection error handling (#19222)
- feat(ramp): display buy button always cp-7.55.0 (#19186)
- feat(perps): improve Perps UX with connection grace period on navigation (#19177)
- feat(perps): Migrate token icons to expo-image for improved performance (#18956)
- feat(perps): implement live funding rate update (#19044)
- feat(ramp): send using networkclientid cp-7.55.0 (#19067)
- feat(ramp): set evm only for sell address selector cp-7.55.0 (#19069)
- feat(perps): updated perps skeleton TAT-1399 (#19025)
- feat(peprs): dynamic fee calculation (#19054)
- feat(perps): design adjustments (#19051)
- feat(deposit): upgrade sdk to v2 (#18524)
- feat(deposit): add address selector cp-7.55.0 (#19060)
- feat(sdk+wc): move sdk & wc init to saga (#17436)
- feat(ramp,deposit): add network badge to orders cp-7.55.0 (#18994)
- feat(ramp,deposit): show orders tab always cp-7.55.0 (#18993)
- feat(e2e): enforce consistent test setup with withFixtures() (#18998)
- feat(pr template): make changelog entry mandatory (#18638)
- feat(deposit): add mUSD token to deposit via feature flag (#18981)
- feat(deposit): updates home page CTA to send users to deposit page (#18852)
...
- feat(perps): update Perps withdrawal screen and remove Hyperliquid network (#18763)
- feat(card): add basic cache strategy with Redux (#18587)
- feat(multichain-account): update wallet details to state 2 and add hideMenu prop to AccountCell and update tests (#18613)
- feat(perps): add deeplink support for perps (#18568)
- feat(card): implement skeleton loading states (#18541)
- feat(assets): add account group balance selectors and components (#18315)
- feat(multichain): add MultichainAddWalletActions component and related types, styles, and tests (#18462)
- feat(multichain-accounts): Add AccountGroupDetails component with styles and subcomponents (#18569)
- feat(base branch): make it possible to define name of release PR base branch (#18495)
- feat(INFRA-2845): restrict release pr merges (#18349)
- feat(perps): migrate to react-native-safearea-context for android 35 (#18547)
- feat(perps): improve hyperliquid fee estimation based on trading and staking volume (#18544)
- feat(card): rename add funds bottomsheet options (#18366)
- feat(perps): implement stream architecture for optimized rendering performance (#18383)
- feat(SDKConnectV2): UI integration (#18760)
- feat(SDKConnectV2): Headless Connection Handling (#18752)
- feat(perps): add missing icon in perps funding history (#19371)
- feat(perps): optimize Intl formatting performance to reduce tab switc… (#19438)
- Add rewards referral view & rewards home view (#19081)
- add(SDKConnectV2) - Scaffolding (#18305)
- Feat/change-password-error-prompt (#19523)
- feat: marketing opt in API (#19653)
- feat: rewards with multi account subscriptions (#19739)
- feat: update textBox design in case of error. (#19664)
- feat: TAT-1664 user can set a stop loss below liquidation price (#19714)
- feat: TAT-1659: Updated perps full screen modal announcement (#19716)
...
- feat: TAT-1671 remove order type button when closing position (#19695)
- feat: update copy of perps deposit alerts (#19631)
- feat: add multichain accounts intro modal and related actions (#19594)
- feat: metamask pay metrics (#19602)
- feat: TAT-1629 removed token input from close position view and optimized slider (#19628)
- feat: Multi-chain accounts list Icon (#19539)
- feat: hide remove private key during seedlessflow (#18868)
- feat: SL-152 Enable biometrics automatically on both SRP and social login flow during wallet creation and unlock. (#19440)
- feat: update perps tooltip to enable notifications (#19250)
- feat: add Contentful version segmentation (#19529)
- feat: handle deadend relogin (#19290)
- feat: TAT-1335 added funding description to funding tab in perps tx history (#19567)
- feat: Fix Perps Toast Issues (#19563)
- feat: perp push notification deeplinks (#19417)
- feat: add store link on DeepLinkModal for invalid links cp-7.55.0 (#19439)
- feat: cp-7.55.0 add What's New modal for UI improvements announcement (#19361)
- feat: update quote card UI (#18737)
- feat: make MaskIcon the default account icon for all users (#17793)
- feat: multichain account permissions (#19368)
- feat: add rewards activity tab (#19333)
- feat: TAT-1479: replaced max fiat amount with token amount (#19474)
- feat: renew revoke token v2 (#18951)
- feat: allow google login in android to auto retry once (#19373)
- feat: TAT-1553 fix tp sl interface issues (#19473)
- feat: bip44 state 2 multichain accounts feature flag mock and relate test helper (#19464)
- feat: MultichainAccountConnect (#18918)
- feat: TAT-1339: Refactor Perps Toasts (#19367)
- feat: use `MultichainAccountService.discoverAndCreateAccounts` (#19197)
- feat: bip 44 network selector (#19276)
- feat: update @metamask/account-tree-controller to v0.13.0 and adjust error handling in EditMultichainAccountName (#19436)
- feat: show-password-outdated error upon login (#18295)
- feat: rewards onboarding for single account (#19428)
- feat: add tou and privacy policy (#19231)
- feat: Add Sentry performance metrics for SRP account creation and import flows (#19225)
- feat: Add confirmation message after network switch from SDK (#19087)
- feat: refresh metamask pay quotes (#19365)
- feat: Init Perps e2e pattern (#18738)
- feat: cp-7.55.0 gns nft import (#19359)
- feat: move alerts to keyboard in perps deposit (#19301)
- feat: update BottomSheetHeader of MultichainAccountActions and MultichainEditAccountName components (#19242)
- feat: Enable Arbitrum for Perps withdraw and deposit flows (#19272)
- feat: Swaps rewards (#19138)
- feat: Add expo-haptics (#19187)
- feat: bump solana snap (#19312)
- feat: update account creation flow in BaseWalletDetails (#19076)
- feat: update EditAccountName component to work with AccountGroups (#19074)
- feat: automatically increase metamask pay buffer (#19236)
- feat: Perps design review tweaks (#19211)
- feat: use new bip44 token selectors (#18566)
- feat: rewards opt in (#19221)
- feat: add perp push notification translations (#19182)
- feat: update earncontroller initialisation for bip-44 (#19160)
- feat: Use gasUsed from simulation for user facing network fee estimation (#19190)
- feat: TAT-1335 display geo block bottom sheet tooltip when geo blocked user interacts with perps (#18856)
- feat: use the network from cryptocurrency cp-7.55.0 (#19065)
- feat: add perps gtm modal for first time users (#19153)
- feat: trigger MultichainAccountService basic functionality updates (#18472)
- feat: show season status in rewards module (#19154)
- feat: support atomic swaps if payment token on same chain (#18700)
- feat: perps deposit activity (#18537)
- feat: add insufficient payment token native alert (#18717)
- feat: remove chainId filtering from order history cp-7.55.0 (#19055)
- feat: rewards - estimate points, perps fee discount & has opted in (#19036)
- feat: state 2 wrapper component and dapp connection utils (#19024)
- feat: add address selector cp-7.55.0 (#18893)
- feat: cp-7.55.0 turn on remove gns flag (#18988)
- feat: bump solana snap (#18941)
- feat: introduce rewards tab (#18891)
- feat: Enhance FixtureBuilder with account tree controller setup (#18862)
- feat: enhance MultichainAccountSelectorList with walletId support (#18769)
- feat: BIP-44 account support for Earn (#18779)
- feat: MultichainAccountConnectMultiselector (#18800)
- feat: Replace `hostname` with `origin` for permissions and per dapp selected network state (#17523)
- feat: useAccountGroupsForPermissions hook (#18570)
- feat: network manager redesign (#18701)
- feat: Implement `sendRedesign` feature flag (#18872)
- feat: New asset selectors in `asset` page in send flow (#18773)
- feat: MultichainPermissionSummary (#18727)
- feat: add perps balance to total wallet balance, add pnl to perps tab (#18545)
- feat: bip44 assets selector (#18733)
- feat: deprecated hardcoded carousel banners (#18754)
- feat: support selected currency in perps deposit (#18818)
- feat: update layout of perps deposit confirmation (#18385)
- feat: multichain accounts connected list (#18721)
- feat: Implement `balance` for NFTs in BIP-44 send (#18711)
- feat: Swaps UnifiedSwapBridgeEventName.AssetDetailTooltipClicked (#18645)
- feat: add navigation to address list view (#18698)
- feat: rewards-controller (#18582)
- feat: implement navigation to address list in MultichainAccountActions (#18716)
- feat: reveal private key list (BIP-44) (#18441)
- feat: add `gasFeeTokenModal` and `gasFeeTokenListItem` components (#18102)
- feat: Add NFT list in asset page in send flow (#18602)
- feat: Add `SnapUIDropdown` and `SnapUIRadioGroup` (#18468)
- feat: stable delete screen (#18457)
- feat: Submit solana transaction to snap for review and confirmation (#18560)
- feat: add address list (BIP-44) (#18325)
- feat: add useAccountGroupName hook and integrate into Wallet view (#18494)
- feat: Integrate lightweight-charts into Perps (#18418)
- feat: update pay with row design (#18332)
- feat: display total network fee in perps deposit (#18378)
- feat: BIP-44 account support for Perps with seamless Solana account handling (#18546)
- feat: add multichain accounts dapp connection selectors (#18343)
- feat: nonevm deeplink handler (#18497)
- feat: Filter from account in the recipient page (#18489)
- feat: live prices with market data screen (#18455)
- feat: Add `useSnapNameResolution` hook (#18453)
- feat: implement react native release profiler (#18109)
- feat: removal all wizard related logic (#17824)
- feat: Implement multichain account actions and navigation (#18308)
- feat: Changes in amount page for NFT send (#18429)
- feat: optimize Perps account/network monitoring with lifecycle manage… (#18451)
- feat: Add real-time account data streaming for Perps via WebSocket (#18434)
- feat: broke out perps balance modal into its own view instead of rend… (#18439)
- feat: add real-time WebSocket streaming to Perps with performance optimizations (#18430)
- feat: as a user i can subscribe to push notifications (#18165)
- feat: TAT-1306: add image animation to tutorial (#18397)
- chore: Remove arrow next to available balance icon (#19723)
- chore: Updated the homepage carousel with the new stacked cards (#19607)
- chore: Updated Buy Button's label and icon (#19629)
- test: Multichain create new account e2e test (#19465)
- chore: Enable sentry by default for devs (#19170)
- chore: cp-7.54.2 remove card smoke tests from Android and iOS workflows (#19655)
- chore: add sourcemaps to bitrise artifacts (#19543)
- test: Add new e2e tests - Liquidation + Limits (#19400)
- chore: Enable Bitcoin testnets before BIP44 (#19646)
- chore: Add Solana Devnet feature flag (#19662)
- chore: Capture sentry exceptions during critical integration perps flows (#19721)
- ci: make Android E2E tests optional for now (#19470)
- perf: add trace for account discovery (#19494)
- perf: add missing "Show Account Addresses" trace (#19452)
- chore: cp-7.55.0 add "NaN" check (#19444)
- chore: create temp branch for nightly builds (#19350)
- perf: add performance tracing for multichain accounts (#19218)
- chore: added NCLocation explanation into plist (#18989)
- perf: add trace on address copy (#19522)
- test: add waitForElementToDisappear option to tapManagePermissionsButton (#19390)
- chore: add cursorbot to CLA exception (#19610)
- chore: gradle cache path update (#19326)
- chore: implement iOS runners for e2e tests (#19091)
- chore: cp-7.54.1 add logging to get more info around blocking login biometrics (#18910)
- INFRA-2849: Don't run auto create release pr on hotfix (#19253)
- chore: qa feedback adjust-perps-tab-based-on-user-state-holdings (#19262)
- test: add the MM_REMOVE_GLOBAL_NETWORK_SELECTOR back to the code (#19176)
- chore: Add new temp Tabs component (#19145)
- INFRA-2849:Added auto-create-release-pr workflow to automatically create release cp-7.55.0 (#19090)
- chore: Added ButtonSemantic to components temp (#19132)
- ci: add job id to E2E test reports (#19123)
- chore: migrate swaps to BIP-44 (#18824)
- chore: adds migration to remove value from storage (#19050)
- test: Add APIs used in Snaps tests to allowlist (#19041)
- chore: add `selectAccountGroupsByAddress` and `selectInternalAccountByAddresses` selectors (#18985)
- chore: Added SmartAccount drawer to AccountDetails (#18888)
- chore: Default app start user (#18108)
- test: add custom ci properties for the test reports (#18810)
- test: makes Android E2E mandatory (#18919)
- ci: add job for nightly performance e2e runs (#18895)
- chore: Updated tokens order in token list items (#18289)
- New Crowdin translations by Github Action (#17879)
- chore: Added ShareAddressQR code to AddressList and aligned QR screens (#18836)
- test: add remote feature flags mock in server setup (#18876)
- chore: cp-7.54.0 buy view switches to param chainId (#18879)
- chore: Improve metrics for failed network requests (#18059)
- chore: cp-7.54.0 - deeplink- use the native asset for the passed chain (#18765)
- test: add regression workflow (#18780)
- INFRA-2867: Added chore prefix to PR, await for create release, remove commits.csv from changelog pr, add workflow artifact (#18840)
- chore: bitcoin v1 (#18789)
- INFRA-2867: Update inputs for workflow, update input parameter (#18786)
- chore(card): add metamask card website on dapps-url-list.js file (#18744)
- chore: Bump webview to 14.5.0 (#18736)
- chore: add appwright packages (#18579)
- test: pass testSpecificMocks as function (#18551)
...
- MPW 101 (#19720)
- chore: bump communication layer package of sdk (#19753)
- test: enable profiling skip onboarding for performance (#19447)
- ci: split smoke E2E tests between android and iOS platforms (#18567)
- test: Add E2E for getFile example Snap (#18081)
- test: update default mocks logic and logic to handle live requests in tests (#18437)
- chore: central rca form/sheet (#18482)
- ci: better manage automatic labeling of Sentry issues (#18254)
- chore: update cursor rule PR creation guidelines with auto team label detection (#18288)
- chore: deprecate reusable modal and suggest bottom sheet (#18111)
- test: disables bitrise android e2e and adds GHA e2e to CI (#18316)
- ci: add the build saving/caching to the new templates (#18438)
...
- fix(perps): adjustments in withdrawal screen (#19477)
- fix(perps): issues in close screen (#19471)
- fix(perps): intermittent Perps connectivity issues caused by API rate limit (#19467)
- fix(perps): invalid tif value for market order TAT-1475 (#19374)
- fix(perps): liquidation price for limit order based on invalid entry … (#19376)
- fix(perps): general UI adjustments (#19559)
- fix(perps): keyboard input responsiveness and position close amount calculation in perpetual trading (#19061)
- fix(card): card home not showing asset balance if asset is hidden (#19015)
- fix(ramp): adjust navbar ui cp-7.55.0 (#19068)
- fix(deposit): fixes deposit dob input (#19251)
- fix(deposit): cp-7.55.0 use multichain balance to determine 'add funds' entrypoint (#19264)
- fix(test): general ci flakes (#19196)
- fix(aggregator): fixes small property of undefined bug for some orders (#19126)
- fix(card): cache prevent loading different address (#18997)
- fix(deposit): add reverse lookup for musd (#19047)
- fix(perps): leverage slider liquidation price TAT-1367 (#19026)
- fix(card): failing E2E tests (#18833)
- fix(next semver version): no longer rely on tags to determine next semver version (#18755)
- fix(perps): resolve multiple order flow issues and UX improvements (#18746)
- fix(perps): resolve Perps token logo rendering issues (#18732)
- fix(perps): perps WebSocket lifecycle - tab visibility & app state management (#18703)
- fix(ramp): enable fund menu for bitcoin (#18642)
- fix(ramp): successful order redirection handler (#18647)
- fix(next semver version): revert calculation method to no longer rely on package.json (#18500)
- fix(perps): initial qa feedbacks (#18490)
- fix(INFRA-2932): retry yarn packages installation (#19491)
...
- fix(action): be more explicit about the fact that parameter can be a tag OR a commit hash (#18255)
- fix swap button (#18803)
- Fix: Internet connection not available check added during social onboarding. (#18706)
- fix: updated border for network selectors to be muted (#19756)
- fix: tp sl solution in perps proposal B (#19726)
- fix: cp-7.55.0 Fix previous confirmation shown in send flow (#19737)
- fix: currency conversion and display on amount page (#19729)
- fix: minimum amount check in perps deposit (#19738)
- fix: vertical scroll issue with tabslist (#19724)
- fix: Hardcode Perps USDC withdrawa token image url (#19706)
- fix: cp-7.55.0 fix unstake regression (#19710)
- fix: tabs spacing and behavior (#19689)
- fix: animation on lock screen, replace by static fox more loader (#19342)
- fix: close position limit display info is based on market price (#19643)
- fix: cp-7.55.0 Defer network setup until SRP import (#18125)
- fix: More perps chart adjustments (#19688)
- fix: Add whitelisted actions to universal link interstitial (#19633)
- fix: Fix chart unresponsiveness on Android when scrolling (#19652)
- fix: cp-7.55.0 fiat amount input width when toggling fiat mode (#19660)
- fix: cp-7.55.0 token name display in token hero component (#19647)
- fix: All UI fixes/polishing live coded (#19614)
- fix: cp-7.55.0 initialize network enablement controller (#19204)
- fix: withdraw max validation (#19621)
- fix: add back button to settings view (#19527)
- fix: cp-7.55.0 send flow broken at times if started from asset detail page (#19613)
- fix: cp-7.55.0 catch exception to prevent app crashing in certain failed Swaps (#19561)
- fix: use hash as txHistory key and txMetaId for Solana bridges (#19270)
- fix: short positions can now find tpsl (#19587)
- fix: flipped position pnl display in transaction list (#19582)
- fix: cp-7.55.0 reset recipient page as solana request is cancelled (#19599)
- fix: cp-7.55.0 fix grant permission via connected site popover (#19456)
- fix: hide metamask pay in activity (#19497)
- fix: Keyboard dismiss on valid seedphrase (#19297)
- fix: log non critical info using log instead of error (#19384)
- fix: cp-7.55.0 currency conversion possibly resulting in error (#19584)
- fix: Unnecessary truncation for MainActionButton cp-7.55.0 (#19188)
- fix: slider cannot go to 100% on order form without error (#19580)
- fix: return a 7 day fundings range to avoid clipping (#19575)
- fix: cp-7.55.0 in fiat mode only 2 decimals should be allowed in amount (#19578)
- fix: tutorial logic now consistent and avoids loops (#19568)
- fix: cp-7.55.0 amount page max decimals related fixes (#19519)
- fix: ui price formatting and input logic (#19512)
- fix: TAT-1518 - Keyboard width causes wrapping issues on buttons (#19565)
- fix: Funding cost bug (#19531)
- fix: Unresponsive PerpsTabView touchable opacities (#19556)
- fix: TAT-1556 - Liquidation distance sometimes wrap incorrectly (#19564)
- fix: When changing candle periods, zoom to latest candle (#19540)
- fix: Selected avatar account type styles in settings page (#19554)
- fix: cp-7.55.0 fix aggregated balance when toggle off (#19548)
- fix: TP/SL presets wrap incorrectly (#19510)
- fix: false warnings for HyperEVM (#19167)
- fix: new network filter behaviour fix for bip44 selector (#19499)
- fix: cp-7.55.0 fix token selector for send flow (#19513)
- fix: update icon for account details action in MultichainAccountActions component (#19511)
- fix: disable keyboard avoiding view in AccountSelector component (#19501)
- fix: cp-7.55.0 sending ERC721 in new send implementation (#19502)
- fix: cp-7.55.0 recipient input not working after selection from list fails (#19490)
- fix: rewards error modal navigation (#19480)
- fix: lint issues with test perps card (#19482)
- fix: Order press should navigate to order tab by default (#19468)
- fix: not allow delete selected network (#19462)
- fix: left aligned account picker in all cases for wallet homepage (#19426)
- fix: cp-7.55.0 use home page network filter for default swaps pair (#19398)
- fix: Improve Perps Chart panning navigation (#19345)
- fix: cp-7.55.0 adding sns name resolution and validation (#19430)
- fix: perps e2e tests (#19432)
- fix: Funding cost (#19427)
- fix: cp-7.55.0: earn lending deposit confirmation button disabled state (#19399)
- fix: removed protect wallet (#19386)
- fix: android multichain account list scroll (#19431)
- fix: cp-7.55.0 deep link transfer request cause app to crash (#19382)
- fix: cp-7.55.0 show scroll if amount page length exceeds 100% (#19406)
- fix: tutorial navigation issues (#19355)
- fix: Update design system package to fix avatar generation cp-7.55.0 (#19396)
- fix: TAT-1537 Added missing geo block bottom sheet when using clicks "Add Funds" on PerpsMarketDetailsView (#19407)
- fix: cp-7.55.0 amount input related fixes (#19380)
- fix: notify the client when native balance goes to zero (#19389)
- fix: cp-7.55.0 Prevent adding multiple txs in the send flow (#19263)
- fix: hide Solana in metamask pay (#19366)
- fix: reset scroll position on search text change in MultichainAccountSelectorList (#19378)
- fix: Hide Protect Wallet For Social Login Flow cp-7.55.0 (#19331)
- fix: back gesture disabled for SRP Screen (#19307)
- fix: prevent calling wallet_addEthereumChain only on persistent chains (#18958)
- fix: track deposit completion events in orderProcessor rather than in the view cp-7.55.0 (#19329)
- fix: full screen error retry has no back button (#19284)
- fix: cp-7.55.0 UI flickering and ENS name resolution in contact form (#19296)
- fix: cp-7.55.0 Fix token details display in header for native token send (#19298)
- fix: cp-7.55.0 bump assets controllers to v74.3.2 (#19150)
- fix: regression of transaction detail size and P&L display (#19273)
- fix: add recipient `address` and tooltip cp-7.55.0 (#19245)
- fix: cp-7.55.0 network switch navigating to home screen when in send flow legacy (#18996)
- fix: subscriptions lose connection after coming back from background (#19215)
- fix: add BIP-44 check to selectSourceWalletAddress (#19244)
- fix: cp-7.55.0 Fix ENS recipients and recipient validations (#19031)
- fix: e2e test flakes (#19217)
- fix: cp-7.55.0 use fiat conversion rate from asset if available (#19212)
- fix: 'Connection Request Complete' analytics event not firing (#19042)
- fix: cp-7.55.0 max mode should result on 0 value if native asset available is less than gas needed (#19179)
- fix: ignore quote in metamask pay if target not met (#19161)
- fix: cp-7.55.0 Fix NFT component presentation (#19070)
- fix: cp-7.55.0 max mode implementation for new send flow (#19084)
- fix: Hide send max on OP stack (#18756)
- fix: metamask pay quotes gas (#19075)
- fix: cp-7.55.0 fixes in getting balance for assets in send flow (#19037)
- fix: cleanup prewarm on account switch in stream classes (#19013)
- fix: cp-7.55.0 Show testnet balances regardless of the fiat value (#19023)
- fix: cp-7.55.0 Fix app crash when resolving account_type after idle (#19043)
- fix: Enable Snaps WebView debugging in dev (#19039)
- fix: cp-7.55.0 Show imported account and hardware wallets in new send flow (#19022)
- fix: alert flicker in perps deposit (#19011)
- fix: Fix first time empty positions state not loading (#18980)
- fix: cp-7.55.0 fix network manager navigation and delete cases (#18874)
- fix: modal backdrop touch handling when backdropOpacity is 0 (#18367)
- fix: enable network after swaps (#18402)
- fix: cp-7.54.0 Use proper Jazzicon seed for non-EVM (#18917)
- fix: cp-7.54.0 Remove lottie animation from login screen due to slowness (#18938)
- fix: cp-7.55.0 Fix redesigned send flow issues (#18939)
- fix: shorten account name for smaller screen ci flake (#18934)
- fix: bigger input focus area for inputs (#18784)
- fix: infinity spinner for lending deposit cp-7.54.0 (#18887)
- fix: double press interaction on android cp-7.54.0 (#18729)
- fix: update assets controllers with fix (#18884)
- fix: cp-7.54.0 Launchdarkly flag added carousel-banners (#18855)
- fix: stable sync secret (#18867)
- fix: define `transactionType` before adding transaction (#18234)
- fix: stop account tracker polling (#18603)
- fix: smart transactions mobile feature flags #18781 (#18820)
- fix: checkbox reset on No Thanks click (#18748)
- fix: amount validation on send page (#18870)
- fix: Fix group name for Name components (#18865)
- fix: NFT balance related fixes on amount page (#18812)
- fix: remove invalid header for accountbackup page (#18747)
- fix: Bundle lightweight-charts package locally (#18847)
- fix: Reintroduced scan button to Wallet Homepage cp-7.54.0 (#18832)
- fix: WC universal links (#18741)
- fix: Fix network switcher not appearing on legacy Swaps (#18643)
- fix: deescalate google error cp-7.54.0 (#18745)
- fix: cp-7.54.0 solana ws lifecycle (#18759)
- fix: support fallback to sequential batch (#17449)
- fix: Updated MainActionItem and bottom nav's TabItem to reduce padding for small screens (#18734)
- fix: cp-7.54 close solana websockets on inactive (#18718)
- fix: text style update (#18722)
- fix: Update the copy writing of the SRP login button (#18713)
- fix: disable linktext btn Use a different login method (#18607)
- fix: forced light theme colors for buttons cp-7.54.0 (#17756)
- fix: account_status img update (#18609)
- fix: add missing properties in transaction event (#18169)
- fix: create password loading (#18606)
- fix: reset wallet button disabled while loading (#18608)
- fix: SRP image cp-7.54.0 (#18376)
- fix: fix transaction history for new network manager (#18436)
- fix: flickering input srp (#18562)
- fix: cp-7.54.0 update liveness logic (#18364)
- fix: google cancel login in android cp-7.53.0 (#18596)
- fix: enter keydown (#18564)
- fix: Allow most WebView options on all pages (including homepage) (#18477)
- fix: network manager scroll bug for android (#18535)
- fix: cp-7.54.0 use navigation.dispatch to properly navigate to asset page (#18461)
- fix: Reveal Private Credentials cp-7.54.0 (#18484)
- fix: font-size for input value on amount page (#18549)
- fix: Fix send flow hang on custom RPC by signaling blocked XHR (#18304)
- fix: cp-7.53.0 swaps weth fee (#18538)
- fix: user data being wiped after upgrading to new version from 7.51.4 (#18398)
- fix: cp-7.54.0 remove network/account filters from deposit order selector (#18499)
- fix: cp-7.54.0 quotes on Optimism crashing app (#18486)
- fix: Implement network selector in asset page for BIP-44 send (#18466)
- fix: fix aggregated balance gns (#18471)
- fix: BIP-44 send flow design updates (#18379)
- fix: Prevent clearing Snap permissions when clearing dapp approvals (#18380)
- fix: Update Perps order keypad buttons to match latest design (#18447)
- chore: bump `account-tree-controller` + `accounts-controller` (#19504)
- test: fix regression tests (#19657)
- test: update failing e2e for per dapp selected network (#19644)
- test: fix failing confirmations e2e (#19648)
- refactor: import 5792 middleware from monorepo package (#19320)
- chore: bump axios to v1.12.0 (#19630)
- chore: improve engine tests (#19535)
- test: update onboarding for performance e2e (#19616)
- chore: Updated Wallet homepage to use custom tabs and enable page-level scroll behavior (#19141)
- ci: align QA labels (#19538)
- test: move tests to RegressionIdentity for account syncing tests (#19596)
- refactor: remove RevealPrivateKey component and related references (#19508)
- chore: swaps rewards error state (#19414)
- chore: sync stable to main for version 7.54.1 (#19536)
- refactor: update perps push translation copy (#19530)
- chore: update `multichain-transaction-controller` to latest version (#19443)
- chore: change detection (#19442)
- test: qurantine and fixing most flaky smoke tests (#19503)
- ci: skip E2E tests when no need (#19507)
- chore: cirruslabs cache (#19441)
- test: Fixed flaky test `e2e/specs/notifications/notification-settings-flow.spec.ts` (#19412)
- test: add job for e2e remove gns disabled (#19319)
- test: re-enable confirmations redesigned tests for Android and iOS (#19395)
- ci: move ios E2E tests to a specific workflow (#19401)
- refactor(card): adapt card feature to use new bip-44 selectors (#18362)
- Mmqa 962 performance aggregate reports (#19305)
- ci: avoid ios E2E tests block PRs (#19381)
- ci: remove needs-triage label when issue has both team and severity (#19362)
- test: fix system dialog popups in performance e2e (#19351)
- chore: swap out all npx calls to use yarn (#19344)
- chore: Replacing npx with yarn to install husky script (#19341)
- test: fi flaky test asset sort (#19287)
- chore: Update `base-controller` to v8.3.0 (#19278)
- chore: Fix label name in docs (#19317)
- test: Added tap to disappear on confirm button (#18902)
- chore: improve quote rate formatting (#19299)
- ci: fix performance workflows for browserstack (#19281)
- chore: nightly build bump version (#19134)
- Mmqa 951 fix swap test (#19140)
- chore: cp-7.54.1 update solana to 2.3.6 (#19234)
- refactor: migrate eip-5792 & capabilities middleware handlers into monorepo (#19064)
- chore: CaipAssetType in points estimation & Accept-Language (#19216)
- chore: small design adjustments to tab view home (#19193)
- refactor: Replace candle intervals with candle periods (#18894)
- chore: bump SonarCloud action version (#19158)
- chore(dev): Use fewer CPU cores to bundle on lesser machines (#16378)
- ci: add flaky tests bot workflow (#19184)
- chore: round source token balance down (#19156)
- chore: adjust perps tab based on positions/orders/balance/firstTimeUser (#19144)
- chore: Bump BridgeController to 41.4, BridgeStatusController to 40.2 (#18903)
- test: Disable bridge test (#19152)
- chore: cp-7.55.0 update solana to 2.3.4 (#19049)
- test: Re-enable bridge smoke and swap chart e2e tests (#18516)
- test: add back quarantined confirmation redesign tests from GNS removal (#19079)
- ci: Bump `setup-ruby` (#19073)
- test: reduce flakey contact sync e2e test (#18957)
- ci: add missing job id in the test report (#19027)
- test: fix import flow for performance (#19014)
- test: fix appwright onboarding (#19012)
- chore: Correctly calculate open interest (#18906)
- chore: delete Solana GTM modal, code and references (#18931)
- test: narrows down Mocking logs and replaces console.logs (#18937)
- chore(deps): bump `@metamask/multichain-transactions-controller` to `^4.0.1` (#18814)
- test: clean up of api-mocking folder (#18897)
- refactor: simplify wallet balance handling in hooks and components (#18875)
- test: fix ui for performance e2e (#18908)
- test: fix swap-action-smoke-spec.ts e2e test (#18898)
- chore: Bump main version to 7.56.0 (#18901)
- test: disables flaky swap test (#18883)
- INFRA-2867: Skip version bump PR creation if already exists, commit in main (#18881)
- test: fail new untracked/unmocked urls in e2e tests (#18724)
- test: add cursor rules for E2E testing (#18087)
- build: ignore unit test coverage files (#18871)
- test: adds missing build step (#18846)
- chore: display `AAVE` icon on earning workflow (#18702)
- chore: Update PerpsPositionCard to include Funding cost (#18858)
- chore(deps): bump `@metamask/signature-controller` to `^32.0.0` (#18806)
- test: skip perps e2e (#19654)
- test: update create account test and add fixture helper to suppress modal (#19743)
- chore: cleaned up AddressList style (#18834)
- test: Adds unit tests for mockHelpers (#18809)
- test: Fix mock request body matching for multiple POST requests to sa… (#18813)
- test: performance e2e suite (#17773)
- refactor: enable multiselect in MultichainAccountSelectorList (#18719)
- chore(deps): bump eip1193-permission, multichain-api, and notification-services controllers (#18726)
- chore: revert contextual chain id prs (#18487)
- test: fixes confirmation test detection (#18730)
- chore: Bump version number to 2296 (#18739)
- ci: fail tests so we can see flakiness on the pipelines (#18708)
...
- test: adds a mock to the favicon to avoid test starvation (#18552)
- refactor: import getCaip25PermissionFromLegacyPermissions and requestPermittedChainsPermissionIncremental from chain agnostic permission package (#17908)
- test: fixes artifact upload on failures (#18479)
- test: fix confirmations default erc20 e2e test (#18518)
- refactor: cleanup unused Perps deposit code and improve deposit status feedback (#18463)
- refactor: refactor `MultichainAddressRow` (#18442)
- test: quarantine flaky browser tests (#18432)
- test: move onboarding tests to somke wallet platform (#19755)
- test: Fix Per dapp selected network e2e to include GNS disabled scenario (#19750)
- test: Fixed test `e2e/specs/settings/addressbook-tests.spec.ts` (#19622)
- chore: Bump main version to 7.55.0 (#18401)
- chore: bump assets controllers to v73.3.0 (#18483)
- chore: bump shajs and cipher base (#18578)
...
- feat(card): rename add funds bottomsheet options (#18366)
- feat(perps): implement stream architecture for optimized rendering performance (#18383)
- feat(deposit): add linea supported tokens for cardholders (#18361)
- feat: useAccountGroupsForPermissions hook (#18570)
- feat: network manager redesign (#18701)
- feat: Implement `sendRedesign` feature flag (#18872)
- feat: New asset selectors in `asset` page in send flow (#18773)
...
- feat: support selected currency in perps deposit (#18818)
- feat: update layout of perps deposit confirmation (#18385)
- feat: implement search functionality in MultichainAccountSelectorList (#18242)
- feat: fix arithmetic used for non-evm send on amount page (#18296)
- feat: improve perps deposit totals (#18252)
- feat: UX related improvements in send amount page (#18293)
- feat: Adding metrics for new send implementation (#18237)
- feat: When user clicks "View on block explorer" open Reborn browser instead of SimpleWebView (#17912)
- feat: Add more E2E tests for the Browser: (#17460)
- feat: get all internal accounts for a given scope (non user facing change) (#18275)
- feat: transition bridge controllers to new init pattern (#17676)
- feat: add MultichainAccountSelectorList (#17904)
- feat: Network Manager UI (#16642)
- feat: multichain account selectors (#17766)
- feat: Centralise send navigation bar (#18192)
- feat: new selectors and hooks for the network enablement controller (#18151)
- feat: as a user i can view the perp tutorial and first time user state (#18113)
- feat: automatically select transaction pay token (#18106)
- feat: add tracing for navigation (#18168)
- feat: component library updates for the Network Manager (#18149)
- feat: add `GasFeeTokenIcon` component (#17614)
- feat: network enablement controller (#18052)
- feat: Perps chart improvements (all pricing lines, x-axis time intervals, y-axis price info) (#18043)
- feat: Session Expiry Bottomsheet added for social login flow only (#17689)
- feat: perps deposit confirmation keyboard (#18007)
- feat: Implement `asset` page design (#18076)
- feat: Adding custom keyboard to amount page (#18062)
- feat: TAT-1148 implement educational bottom sheets (#18009)
- feat: Improvements in amount page formatting (#17944)
- feat: solana websockets (#17600)
- feat: add alerts to perps deposit confirmation (#17892)
- feat: Add support for fiat conversion for non-evm send (#17906)
- feat: MultichainPermissionSummary (#18727)
- feat: add perps balance to total wallet balance, add pnl to perps tab (#18545)
- feat: bip44 assets selector (#18733)
- feat: deprecated hardcoded carousel banners (#18754)
...
- feat: as a user i can subscribe to push notifications (#18165)
- feat: TAT-1306: add image animation to tutorial (#18397)
- feat: add performance tracking and analytics for Perps (#18067)
- feat: get quotes directly in metamask pay (#18352)
- feat: Implement `recipient` in new send flow (#18346)
- feat: TAT-1298: Perps Screen Design Adjustments (#18212)
- feat: perps builder and referral fee collection (#18276)
- feat: Add Linea and Base for Smart Transactions, bump smart-transactions-controller to v18.1.0 (#18350)
...
- feat: selectors for the contextual network selector (#18042)
- feat: redux infra for contextual chain id for send flow (#18012)
- feat: contextual network picker component (#18014)
- feat: Adding wizard for send flow (#17885)
...
- feat: adding fait conversion option on send page (#17704)
...
- feat: Perps chart improvements time interval, candle period (#17917)
- feat: added temp action list item (#17851)
- feat: wallet selector (#17502)
- feat: price impact warning modal (#17781)
- feat: TAT-1318 Move perps sandbox wallet action into developer options (#17850)
- feat: implement Reactotron (#17780)
- feat: Implement RN Worklets/Background Threads (#17674)
- feat: Adding validations for to address in send flow (#17671)
- feat: decoupling PerpsController error strings for Core Monorepo Integration (#17797)
- feat: TAT-1235 Add "Perps Enabled" and "Perps Maintenance Banner" Remote Flags (#17783)
...
- fix: create password loading (#18606)
- fix: reset wallet button disabled while loading (#18608)
- fix: cp-7.54.0 add global notification mocks to e2e tests (#17830)
- fix: cp-7.53.0 disables flaky E2E tests (#17911)
- fix: fixes account related failures in wallet platform and identity tests (#17863)
- fix: bump @metamask/multichain-api-client to v0.6.5 (#17817)
- fix: cp-7.51.2 solana modal android font (#17829)
- fix: Update how utm params are parsed in deeplinks (#17235)
- fix: Fix padding issue of transactions view (#17804)
- fix: cp-7.51.2 scroll behaviour on Solana New Feature view (#17807)
...
- test: add default mocks (#18384)
- chore: Bump main version to 7.55.0 (#18401)
- chore: stable sync main 7.51 hotfixes (#18396)
- chore: Fix selector & key prop warnings on home page (#18164)
- chore: Polished Bottom Nav's dimensions and positions (#18373)
- chore: Update styling for BottomNav (#18285)
- test: add provider initialization, accountsChanged, and chainChanged event tests (#18202)
- ci: adapt the post-merge-validation job to track progress in a sheet (#18353)
- chore: Additional polish for Homepage Redesign (#18328)
- chore: add variant prop to BottomSheetHeader (#18284)
- docs: Add deeplink docs for universal links (#18219)
- chore: add `selectSelectedAccountGroupId` and `selectInternalAccountListSpreadByScopesByGroupId` selectors (#18326)
- feat(gh-action): add reusable workflow for Android and iOS build change detection (#18142)
- feat(card): add useIsCardholder hook (#18355)
- chore: bump earn-controller package to 5.0.0 and update EarnController initialisation and methods (#17445)
- test: Skip flaky tests (#18347)
- test(e2e): Snaps UI link E2E (#18301)
- chore(next semver version): align implementation on extension repo (#18251)
- chore: add fraxtal network and frax icon (#18188)
- chore: add QuickActionButtons component (#18218)
- chore: adding quick action button component (#18324)
- feat(deposit): submit basic info and address kyc forms separately (#17873)
- chore: HeaderBase component improvements (#18314)
- chore: update jest config to ignore storybook files (#18270)
- chore: updating bottom sheet stories (#18278)
- test: Add preinstalled Snap E2E (#18307)
- chore: update Android build config to use separate RC keystore (#18279)
- test: Add Snaps image E2E (#18300)
- test: Add Snap management E2E (#18257)
- chore: update eslint to ignore Text shadow (#18277)
- chore: create pr creation guidelines cursor rule (#18271)
- chore: update storybook documentation with yarn command (#18267)
- chore: add align prop to HeaderBase component (#18210)
- test: fix failing regression e2e (#17974)
- chore: use bridge API instead of swaps API for topAssets call (#18213)
- chore: updated Trade Action Menu to use ActionListItem (#17848)
- test(e2e): Add error snap E2E tests (#18243)
- feat(TAT-575): add fixture to import random generated accounts (#17849)
- test: Add Snaps cronjob E2E (#18244)
- test: Add Snaps JSX E2E (#18235)
- chore: update `SEI` network's name and RPC name from `Sei Network` to `Sei Mainnet` (#18167)
- feat(card): add card home UI (#17082)
- refactor(deposit): change default payment method cp-7.53.0 (#18211)
- chore: Updated Wallet's Buy interaction to Fund Action Menu (#17923)
- chore: unifying Keypad UI across mobile - part 2 (#18055)
- feat(4173): Send flow with contextual chain selector (#13938)
- chore: Enable Why did you render dev tool (#18160)
- test: Clean up e2e & tweak tests for the GHA migration (#18090)
- chore: add debugging with React Native DevTools (#17995)
- chore(snaps): Bump Snaps packages (#17943)
- chore: confirmation keypad updates (#18153)
- chore(deps): multichain dependencies (#18127)
- chore: add `offramp` and `wallet_notify` tests to quarantine (#18193)
- chore: remove state 1 feature flag from `AccountTreeInitService` (#18166)
- ci: adapt PR size job to have permissions to add labels (#18158)
- chore: add multichain accounts local flag (#18056)
- feat(deposit): cp-7.53.0 use order status description from API (#18057)
- feat(bridge): add deep link support for bridge functionality (#17187)
- chore: unifying Keypad UI across mobile - part 1 (#17782)
- chore(card): add redux card slices on CODEOWNERS file (#18129)
- chore(deps): bump multichain deps (#17734)
- test(e2e): Add E2E test for Snap background events (#18123)
- feat(card): add logic to show Card button (#17684)
- test: adds coverage for segment events on confirmations redesigned (#18097)
- test: nft details spec flakey failures (#17940)
- github actions concurrency (#18096)
- chore: update BottomSheet border radius (#17790)
- docs: improve storybook setup instructions and usage guide (#18103)
- chore: Updated AssetDetailActions to use MainActionButton cp-7.53.0 (#18091)
- ci: temporarily disabling ios performance measurement pipeline (#18098)
- refactor(deposit): standardize header navigation buttons (#18095)
- fix(deposit): start dob picker with value (#18031)
- chore: perps transaction history design audit (#18061)
- ci: update job to use fixtures with browserstack devices (#18048)
- feat(qa-build): update pipeline for generating ios and android builds (#17601)
- test(e2e): Add E2E test for Snap dialogs (#17504)
- perf: Update FlashList to v2 cp-7.53.0 (#17789)
- chore: Make Asset sort e2e test more robust cp-7.53.0 (#18046)
- feat(deposit): cp-7.53.0 show recipient address in bank details screen (#18051)
- chore: upgrading `@metamask/design-system-react-native@0.3.0` (#18044)
- ci: add RCA-needed label on RCA workflow (#18032)
- feat(deposit): add networks filtering to token selector (#17996)
- ci: post validation bot to add a checklist in `feat` and `perf` PRs (#17937)
- test: mocked e2e automated tests for monad and megaeth (#16909)
- chore: Send code refactoring (#17729)
- chore: map android signing-configs to auth environments for flask and main (#17868)
- update: Aligned start icon with label (#18008)
- chore: Added disabled state to ActionListItem (#17993)
- chore: upgrading design system libraries (#17872)
- refactor: Use `withFixtures` for Snaps tests (#17894)
- feat(card): add card components (#17508)
- test(e2e): Add E2E test for Snaps WASM (#17932)
- refactor: Simplify iOS simulator build script by using an array for d… (#17802)
- fix(deposit): cp-7.53.0 crypto currency property in event (#17905)
- chore: sonar cloud step to run only on PR (#17902)
- chore: suggest gherkin syntax for test description (#13227)
- ci: Only do required setup for CI step (#17900)
- fix(deposit): cp-7.53.0 make manual transfer orders unique (#17875)
- chore: skip sonar on failure (#17899)
- feat(deposit): prefill personal details form with user data (#17770)
- New Crowdin translations by Github Action (#17762)
- chore: Revert "feat: Implement RN Worklets/Background Threads" (#17674)
- test(e2e): Add Snaps E2E tests (#17416)
- chore: remove unnecessary permissions from update latest build version yml (#17784)
- chore: Polish homepage UI (#17794)
- test: fix fixture server paths (#17792)
- chore: Bump main version to 7.54.0 (#17787)
- feat(deposit): route to build quote after otp authentication (#17776)
...
- fix: cp-7.53.0 add account title mismatch (#18363)
- fix: updated asset hooks to fetch enabled networks only when feature flag on (#18280)
- fix: Bump expo patch version to fix Android crasher - cp-7.53.0 (#18321)
- fix: cp-7.53.0 swaps show effective gas (#18318)
- fix: sanitize seedless controller state - cp-7.53.0 (#18299)
- fix: Updated Wallet Header's alignment (#18323)
- fix: srp input text cuttoff, ios focus issue, back focus on click (#18222)
- fix: Hide `origin` from MM originated confirmations (#17820)
- fix: handle disable seedless for e2e (#18124)
- fix: adjust color and alignment of source network overflow (#18215)
- fix: reorganize migration file (#18233)
- fix: OnboardingSuccess Social Login Update (#18161)
- fix: import srp loading indicator (#18116)
- fix: fix sev1 issues network manager (#18249)
- fix: add is enabled network function (#18260)
- fix: prevent network label overflow (#18190)
- fix: cp-7.53.0 select non-evm transactions irrespective of account (#18247)
- fix: cp-7.53.0 solana content page button color in dark theme (#18206)
- fix: Tweak send max calculation to use max possible actual gas fees (#18128)
- fix: cp-7.51.4 skip bridge and bridgeApproval tx types in STX (#18207)
- fix: cp-7.53.0 Solana onboarding modal - make the buttons visible at the bottom (#18196)
- fix: throw on response not ok - cp-7.53.0 (#18221)
- fix: rehydrate with wrong password causing user stuck at lock - cp-7.53.0 (#18155)
- fix: cp-7.53.0 block continue button if user does not have enough gas (#18150)
- fix: cp-7.51.4 clean up existing smartTransactions state since we don't persist new ones anymore (#18195)
- fix: cp-7.51.4 feature flag solana modal (#18133)
- fix: cp-7.51.4 wipe smart transactions on account reset (#18174)
- fix: cp-7.53.0 - #17929 Revert privacy screen PR (#18156)
- fix: cp-7.51.4 bump @metamask/smart-transactions-controller to 18.0.0 (#18181)
- fix: address flaky test and ensure cleanup for other tests (#17619)
- fix: cp-7.53.0 use flatlist for android (#18100)
- fix: cp-7.53.0 native token balance updates and optimise controller calls (#18092)
- fix: Social Login Link Update (#18049)
- fix: UI Bugs (#18093)
- fix: Fix flickering alert trigger in batched transaction confirmation (#18084)
- fix: usd values in simulation metrics (#17728)
- fix: Market price should render market price, not liquidity price (#18101)
- fix: cp-7.53.0 hide price impact for wrapping quotes (#18078)
- fix: Revert Android SDK bump to fix nav overlay issue (#18105)
- fix: remove the old js utils (#17847)
- fix: cp-7.53.0 update styles for SafeAreaView and ScrollView to ensure proper layout (#18089)
- fix: cp-7.53.0 handle unrecognised Blockaid reasons in confirmation alerts (#18077)
- fix: face id run twice during rehydration (#18036)
- fix: optinmetric screen show up even metric accepted (#18030)
- fix: next version fix label PR (#17822)
- fix: add `account_type` property to the Wallet Created event cp-7.53.0 (#18024)
- fix: remaining time format cp-7.53.0 (#17801)
- fix: QA Bugs cp-7.53.0 (#17976)
- fix: action view update to get the scrollview cp-7.53.0 (#17987)
- fix: synced srp discover accounts - cp-7.53.0 (#17925)
- fix: oauth uat config (#17811)
- fix: cp-7.53.0 swaps default slippage (#17880)
- fix: cp-7.53.0 swaps solana tx history (#17870)
- fix: Make accounts list cell not clickable cp-7.53.0 (#17876)
- fix: disable failing network expansion e2e (#17935)
- fix: cp-7.53.0 Remove `assetId` when undefined (#17927)
- fix: cp-7.53.0 Properly stop Snaps when clearing state (#17810)
- fix: cp-7.53.0 e2e misusing checksum addresses (#17896)
- fix: cp-7.53.0 add global notification mocks to e2e tests (#17830)
- fix: cp-7.53.0 disables flaky E2E tests (#17911)
- fix: fixes account related failures in wallet platform and identity tests (#17863)
- fix: bump @metamask/multichain-api-client to v0.6.5 (#17817)
- fix: cp-7.51.2 solana modal android font (#17829)
...
- fix: Unnecessary truncation for MainActionButton (#19612)
...
- fix: Fix padding issue of transactions view (#17804)
- fix: Hide `origin` from MM originated confirmations (#17820)
- fix: Loading state bug (#14235, #14096)
- fix: Market price should render market price not liquidity price (#18101)
- fix: Tweak send max calculation to use max possible actual gas fees (#18128)
- fix: Tweak send max calculation to use max possible actual gas fees (… (#18258)
- fix: UI Bugs (#18093)
- fix: Update how utm params are parsed in deeplinks (#17235)
- fix: Updated Wallet Header's alignment (#18323)
- fix: add is enabled network function (#18260)
- fix: additional app icons and remove tint fields (#13543)
- fix: adjust color and alignment of source network overflow (#18215)
- fix: adjust ui entry point logic to work for diff flag variations (#16577)
- fix: changelog (#17021)
- fix: changelog issue (#17018)
- fix: copied to clipboard alert not dismissable (#18047)
- fix: default to main code fence and opt-in for beta (#17076, #16599)
- fix: disable failing network expansion e2e (#17935)
...
- fix: face id run twice during rehydration (#18036)
- fix: fix request payments token list (#16960)
- fix: fix sev1 issues network manager (#18249)
- fix: handle disable seedless for e2e (#18124)
- fix: import srp loading indicator (#18116)
- fix: next version fix label PR (#17822)
...
- fix: prevent network label overflow (#18190)
- fix: ramps: change token to tap and confirm (#14238, #14197)
- fix: remove the old js utils (#17847)
- fix: reorganize migration file (#18233)
- fix: rm token detail option from navbar when on solana (#15022)
- fix: srp input text cuttoff ios focus issue back focus on click (#18222)
- fix: update patch to use old AccountTrackerController data structures (#15037)
- fix: usd values in simulation metrics (#17728)
...
- feat: adding option to send max amount on send page (#17621)
- feat: Bump react-native-webview to 14.4.0 (#17765)
- feat: enable notifications by default (#17700)
- feat: Amount validation on send page (#17554)
- feat: transaction bridge quotes (#17408)
- feat: skip await for multichain account discovery (#17730)
- feat: migrate remaining tests to the new fixtures - Part 3 (final) (#17752)
- feat: Added Maskicon to AvatarAccount (#17723)
- feat: Use `EditSpendingCapModal` for batch approval editing (#17015)
- feat: add market details screen with real-time statistics (#17615)
- feat: adding context to send pages (#17453)
- feat: part 2 of the fixtures migration (#17681)
- feat: Metamask mobile perps withdrawal (#17545)
- feat: account cell (#17361)
- feat: multichain accounts service initialization (#17500)
- feat: Add ButtonHero component with primary colors and pressed states (#17579)
- feat: transaction pay token source amounts (#17543)
- feat: account discovery bitcoin (#17566)
- feat: migrate tests to new fixture files - Part 1 (#17670)
- feat: seedless controller update cp-7.52.0 (#17623)
- feat: enable seedless onboarding by default cp-7.52.0 (#17540)
- feat: Bump targetSdk to 35 on Android (#17228)
- feat: fixes fixtures and migrates solana tests (#17572)
- feat: Perps tab navigation (#17532)
- feat: Create a README explaining how to test Segment events on mobile for an RC (#16828)
- feat: srp and key sync cp-7.52.0 (#17356)
- feat: Add missing time ranges for price graph (#17491)
- feat: cp-7.52.0 Enable hardware accounts for transaction confirmations (#17411)
- feat: Add spinner instead of whitescreen for fullscreen confirmations (#17443)
- feat: Pass the asset to send page using routing (#17415)
- feat: Align activity with Bitcoin transactions (#17446)
- feat: change no positions message (#17130)
- feat: migrate Solana account tests to ts, remove flaky swap assertion (#17524)
- feat: perps order creation (#17401)
- feat: 2.11 Sentry error integration cp-7.52.0 (#17160)
- feat: new password sync flow (#17469)
...
- feat: 2.12 backup keyring key instead of password cp-7.52.0 (#17106)
...
- feat: extend E2E Framework logger (#17389)
- feat: 2.7 Chore/sentry onboarding cp-7.52.0 (#16232)
- feat: converts viewHelper and all POMs + selectors to ts (#17376)
- feat: create typescript fixture handling under framework (#17325)
- feat: Filter out duplicate bridging transactions on activity list (#16321)
- feat: add Hyperliquid perpetuals trading integration with deposit functionality (#17265)
- feat: Fix to persist Biometrics on manual lock (#17139)
- feat: unified swaps max button (#17300)
- feat: track kyc analytics deposit (#17312)
- feat: 2.9 password sync update (#16441)
- feat: 2.8 Chore/segment onboarding (#17319)
- feat: add metametrics to backup and sync modal (#17281)
- feat: Perps CandlestickChart and Asset logo (#17040)
- feat: migrate browser tests to the new framework (#17278)
- feat: add svg support to avatar token (#17069)
- feat: gasless unified swaps (#16851)
- feat: perps scaffolding minimal (#17080)
- feat: enable EIP-5792 methods on Multichain API (#16873)
- feat: Add the expo QA pipeline (#16264)
...
- feat: Add page for user to opt in for smart account (#16917)
- feat: authenticate users if basic functionality is enabled (#17027)
- feat: @lavamoat/react-native-lockdown (#16341)
- feat: bitcoin to beta build (#16956)
- feat: 2.3 seedless oauth flow (#15883)
- feat: Add ESLint rules for E2E testing (#16987)
- feat: add fixtures for identity tests (#16940)
- feat: cp-7.51.0 add swap/bridge performance tracing (#16818)
- feat: Show interstitial for public/private/invalid deeplinks (#16481)
- feat: implement privacy screen when app is inactive (#17303)
- feat(deposit): poll for user's kyc status using separate endpoint (#17761)
- feat(card): add Card translations (#17581)
- feat(multichain-accounts): add MultichainAddressRowList component (#17492)
- feat(INFRA-2772): updated release workflows to eliminate confusion (#17727)
- feat(deposit): fixes otp box styling bug (#17632)
- feat(deposit): moves deposit button to the first in the wallet action list (#17739)
- feat(deposit): adds onSubmitEditing prop to email input (#17724)
- feat(deposit): adds a modal for unsupported states (#17514)
- feat(deposit): use logout method (#17525)
- feat(deposit): adds modal for SSN description (#17627)
- feat(deposit): refactor token vault to use consumer keychain (#17535)
- feat(INFRA-2766, INFRA-2763): e2e smoke tests (#17149)
- feat(deposit): display reason for payment failure (#17584)
- feat(snaps): Add `AccountSelector` (#17364)
- feat(deposit): design and copy updates (#17577)
- feat(deposit): transak geolocation (#17008)
- feat(deposit): add more tokens support conditionally on balance (#17513)
- feat(deposit): new BuildQuote heading (#17473)
...
- feat(deposit): adds basic info kyc analytics events (#17337)
- feat(gh-action): add stable sync workflow (#15405)
- feat(deposit): copy and design parity (#17324)
- feat(deposit): adds additional verification view before kyc (#17311)
- feat(deposit): email login analytic events (#17299)
- feat(INFRA-2772): update release workflow to include version bump to main (#17196)
- feat(deposit): update verify identity ui (#17262)
- feat(deposit): get started design updates (#17296)
- feat(deposit): adds analytics to deposit build quote page (#17242)
- feat(TAT-575): add fixture to import random generated accounts (#17205)
- feat(card): add sdk hooks (#17047)
- feat(deposit): fix back navigation and route to Verify Identity page (#17192)
- feat(deposit): Wire payment method (#16845)
- feat(deposit): surface errors (#17173)
- feat(deposit): update feature flags (#17252)
- feat(card): add card sdk (#17156)
- feat(deposit): add payment method filtering (#17085)
- feat(deposit): webview modal component (#17036)
- feat(deposit): error view (#16947)
- feat(deposit): fix android dob input; improve autofill UX (#17001)
- feat(deposit): update kyc processing copy and cta (#16961)
- feat(deposit): route to correct view after provider login via email otp (#16867)
- feat(deposit): adds deposit region to redux (#16849)
- feat(deposit): add privacy section (#16946)
...
- fix: onboarding flash loading on oauth login (#17732)
- fix: refactoring in send code (#17551)
- fix: bridge status polling indefinitely (#17396)
- fix: cp-7.52.0 dapp permissions not properly updating after hardware wallet is forgotten (#17679)
- fix: speed up bridge token picker (#17578)
- fix: Dismiss Snap approvals immediately as a workaround for crashes (#17740)
- fix: smart account upgrade carousel should not be visible if solana account is selected (#17735)
- fix: wallet view race condition cp-7.52.0 (#17692)
- fix: update accounts before account-tree init (#17683)
- fix: cp-7.52.0 qr code unified swaps not working (#17682)
- fix: cp-7.52.0 Reject confirmation if action navigating outside of confirmation (#17678)
- fix: remove overreliance on tokenSnapshot in withdraw and deposit confirmation (#17629)
- fix: fix token detection test (#17699)
- fix: swallow social error (#17644)
- fix: removed protect wallet for social login (#17602)
- fix: DAPP deeplink (#17634)
- fix: Fix WebView postMessage sendActiveAccount issue (#17261)
- fix: avoid crash when there is no chain id passed to Network Controller (#17617)
- fix: standard kyc by better checking purpose of usage (#17583)
- fix: cp-7.52.0 swaps mobile hw tx (#17574)
- fix: URL spoofing on iOS with a redirect to http website. (#17512)
- fix: handle potential undefined primary keyring when the app is locked cp-7.52.0 (#17559)
- fix: hardware wallet removal cp-7.52.0 (#17549)
- fix: fix flake on reset password flow (#17548)
- fix: ActionView Buttons cp-7.52.0 (#17414)
- fix: cp-7.51.1 "send" deeplink regression (#17519)
- fix: Terms of Use aligned with the new design cp-7.52.0 (#17447)
- fix: Removed Login Error cp-7.52.0 (#17476)
- fix: cp-7.52.0 always navigate to tx activity view after swap submission (#17495)
...
- fix: Remove setting for incoming transaction toggle (#17320)
- fix: fix flask e2e smoke tests pipeline (#17423)
- fix: Handle messages from Android and iOS WebViews which detect iFrame urls and send them to React Native app (#16706)
- fix: Base Network change toast and Network Settings (#17409)
- fix: prevent subdomain confusion attacks in RPC domain validation (#17234)
- fix: Remove password input on backup SRP cp-7.52.0 (#17363)
- fix: in-app browser close button visibility (#17381)
- fix: add padding to in-app browser text (#17387)
- fix: correct RC build workflows to use RC configurations instead of prod (#17382)
- fix: Import private Key UI update cp-7.52.0 (#17206)
- fix: cp-7.51.0 legacy swaps input amount small (#17362)
- fix: add case where commit might be removed from history (#17336)
- fix: cp-7.51.0 token fiat values not available in transaction list (#17327)
- fix: cp-7.51.0 disabled request payment button for Solana (#17294)
- fix: cp-7.52.0 push notification fox icon for high res Android devices (#17341)
- fix: srp input cp-7.51.0 (#17329)
- fix: increate wdio login screen open time (#17313)
- fix: Fix redesigned confirmation send e2e (#17314)
- fix: cp-7.51.0 only show unified tx details screen if swap has data (#17280)
- fix: cp-7.51.0 fiat amounts in transaction list (#17208)
- fix: bump @metamask/solana-wallet-standard to 0.5.1 (#17256)
- fix: cp-7.50.2 Fix undefined formatNetworkRpcUrl (#17301)
- fix: prettier codebase (#17277)
- fix: use prod api instead of dev (#17302)
- fix: cp-7.51.0 add missing base notification properties (#17295)
- fix: cp-7.51.0 fixed missing navbar when navigating from pooled-staking confirmation screen back to input screen (#17290)
- fix: Set up flask production to use new build configs (#17190)
- fix: cp-7.51.0 handle Ledger accounts in transaction approval process (#17165)
- fix: cp-7.51.0 Add flatlist to replace scrollview for earnTokenList android/ios (#17254)
- fix: cp-7.51.0 Fix loading state of transfer info page (#17212)
- fix: eth avatar logo change (#17110)
- fix: show account details as background for multichain account actions (#16863)
- fix: cp-7.51.0 missing gas ticker in transaction details (#17231)
- fix: Remove hardware wallets cp-7.50.2 cp-7.51.0 (#17172)
- fix: add keyboardAvoidingViewEnabled prop to BottomSheet (#17238)
- fix: retrieving configured chains from NetworkController (#17226)
- fix: cp-7.51.0 issue with mainnet isLive being false (#17180)
- fix: cp-7.51.0 import token navigation (#17215)
- fix: export srp to go directly to quiz in account details (#17154)
- fix: cp-7.51.0 move segment event 'SRP Backup Confirmed' to ManualBackupStep2 (#17194)
- fix: wallet link in account details (#16939)
- fix: QR Code Scanner #17107 cp-7.51.0 (#17150)
- fix: Button enabled by default cp-7.51.0 (#17198)
- fix: Fix wrong base64 import in Sentry patch by Margelo (#17188)
- fix: cp-7.51.0 legacy swaps input amount small (#17175)
- fix: STX progress bar stuck (#16980)
- fix: wallet details container. (#17164)
- fix: button styling in ShareAddress and EditAccountName (#16839)
- fix: ios deeplink cold start (#16933)
- fix: Resolve release builds pipelines (#17127)
- fix: date picker component failing regex (#17137)
- fix: Lint warnings 1 (#17134)
- fix: UI Improvement for OnboardingCarousel and let's get-started page cp-7.51.0 (#17052)
- fix: qr code address width (#16866)
- fix: Improve address formatting for CAIP asset types (#17029)
- fix: Fix/expo prebuilds main (#17092)
- fix: enable clicking text to go to link in base account detail (#17056)
- fix: Rebump react native quick crypto to fix Flask e2e (#17084)
- fix: don't redirect when switching to 7702 in multichain accounts design (#16897)
- fix: margin bottom for android reveal srp (#17053)
- fix: Reverted unintended changes to Info.plist (#17045)
...
- fix: OptinMetric UI Issues (#16953)
- fix: wallet details padding for android (#16899)
- fix: use dApp selected network in PPOM requests (#16999)
- fix: cp-7.50.0 crash app when selecting evm acc from solana acc (#17019)
- fix: 2.4 update seedless password change (#15890)
- fix: keyboard pushing screen up during password input (#16898)
- fix: lottie-ios (#16963)
- fix: bottom padding in reveal srp for android (#16864)
- fix: Wallet details with long lists (#16964)
- fix: Fix WebView postMessage issue (#16090)
- fix: cp-7.50.0 fix bridge account selection (#16988)
- fix: fix request payments token list (#16960)
- fix: cp-7.51.0 replace fragment with flatlist so that it's possible to scroll (#16618)
- fix: cp-7.51.0 Improve confirmation designs and accessibility (#16937)
- fix: cp-7.50.0 align sdk connection with permission system (#16460)
- fix: e2e multichain accounts - remove account test (#16908)
- fix(deposit): set max dob to today (#17625)
- fix(deposit): clear otp status when resend (#17334)
- fix(deposit): fixes android region select bug (#17284)
- fix(deposit): disables keyboardAvoidingView for WebView modals (#17243)
- fix(deposit): redirection url (#17222)
- fix(deposit): fixes invalid regex patterns for deposit kyc inputs (#17168)
- fix(bridge): resolve duplicate tokens in destination selector with CAIP normalization (#16924)
- fix(ramp): order id formatting (#16949)
...
- fix: feature flag solana modal (#18225)
- fix: bump @metamask/smart-transactions-controller to 18.0.0 (#18228)
...
- feat: Implement `approve` confirmation methods (#16784)
- feat: Process batched transaction with single nested transaction as regular transaction (#16712)
...
- feat: add `SEI` mainnet network support (#14661)
...
- fix: cp-7.50.2 retrieving configured chains from NetworkController (#17241)
...
- feat: enable react compiler for Nav directory (#15220)
...
- fix: Design QA Bugs 2 (#16737)
- fix: cp-7.50.0 removed local flag and regression on LD flag key (#16903)
- fix: Fix bottom modal flicker (#16886)
- fix: cp-7.50.0 change in LD flag Key ID (#16871)
- fix: only showing 30 tokens for evm (#16812)
- fix: cp-7.50.0 Earn lending blocking UI tweaks (#16844)
- fix: Describe what happened modal is incorrectly displayed in dark mode (#16589)
- fix: Sentry issue "null is not an object" in WebsiteIcon/index.js, line 116: getHost(url).substring(0, 1); (#16835)
- fix: cp-7.50.0 restore deleted backupAndSync keys (#16788)
- fix: cp-7.50.0 batch approval broken for unlimited approvals (#16786)
- fix: Bump version of React-native-webview-mm to 14.2.2 to include fixes for Blob file downloading on both platforms (#16803)
- fix: spelling of "steal" in private key warnings in English locale (#16789)
- fix: not going to bridge tx details for sol swaps (#16771)
- fix: display amount for nested transaction in advance view (#16669)
- fix: cp-7.50.0 Gas fee is now sent to tx controller as hex or undefined (#16761)
- fix: cp-7.50.0 TAT-927: USDT allowance increase on deposit confirmation screen hangs (#16689)
- fix: cp-7.50.0 Mitigate flash of safe withdrawal limit message (#16727)
- fix: Remove QA team entries from CODEOWNERS (#16750)
- fix: cp-7.50.0 Remove lending earnings history button (#16648)
- fix: Addressing UI feedbacks in 7702 functionality (#16740)
- fix: cp-7.50.0 Hide simulations MM originated confirmations (#16717)
- fix: cp-7.50.0 adding send button to Solana asset screen (#15799)
- fix: cp-7.50.0 quarantine analytics e2e test (#16741)
- fix: Design QA Bugs cp-7.50.0 (#16655)
- fix: cp-7.50.0 fix infinite loader and inconsistent `completedOnboarding` value (#16695)
- fix: cp-7.50.0 not redirecting to Solana Swaps UI from Legacy Swaps if they select a Solana account (#16605)
- fix: correct switch / switch back labels in account modal to switch account type (#16675)
- fix: cp-7.50.0 Prevent cronjob state from getting out of sync (#16711)
- fix: wallet connect QR code reading failure (#16684)
- fix: Switch to smart account action should not be available for hardware wallet accounts (#16692)
- fix: Exclude smart transaction approval requests from rate limiting (#16619)
- fix: cp-7.50.0 fix refresh calls (#16694)
- fix: cp-7.47.2 account address missmatch with identity (#16674)
- fix: snaps unable to call wallet_switchEthereumChain when they have no accounts permitted (#16377)
- fix: Fix/bottom sheet notch padding (#16649)
- fix: cp-7.50.0 Fix redesigned transaction confirmations after rejecting legacy transaction confirmation (#16654)
- fix: 13165 fix ledger blind sign error displayed not correctly (#16200)
- fix: Import srp error issue cp-7.50.0 (#16621)
- fix: Pass proper arguments in `eth_requestAccounts` implementation (#16664)
- fix: cp-7.50.0 Fix signed or submitted alert and nonce selection (#16600)
- fix: cp-7.50.0 add control bar for networks with no positions (#16597)
- fix: nit pick unit test for method `isValidNetworkName` (#14500)
- fix: Undefined ticker on activity list (#16626)
- fix: default to main code fence and opt-in for beta (#16599)
- fix: Cleanup duplicate wallet permission handlers in BackgroundBridge. Fix incorrect wallet permission handlers in SnapBridge (#16002)
- fix: cp-7.50.0 subscribe notifications on app startup. (#16547)
- fix: Fix `getItemLayout` parameters (#16615)
- fix: cp-7.50.0 bump @metamask/multichain-api-client to 0.6.3 to handle multichain api not responding on page load (#16590)
- fix: Remove delay for dispatch of INIT_BG_STATE_KEY (#16602)
- fix: cp-7.50.0 use actual origin for `MultichainRouter:handleRequest` call in Multichain API (#16565)
- fix: reveal private credential screen scroll (#16445)
- fix: secure text on blur (#16461)
- fix: migration 085 bug (#16572)
- fix: cp-7.50.0 Fix `ApprovalTagUrl` exception for legacy confirmations (#16495)
- fix: adjust ui entry point logic to work for diff flag variations (#16512)
- fix: turn off autofill (autocomplete) workaround (#16523)
- fix: cp-7.50.0 Fix contract interaction metrics (#16525)
- fix: SuccessErrorSheet Icon Update (#16511)
...
- fix: Revert "feat: add spam filtering for wallet_sendCalls" (#15853)
...
- feat(identity): rebrand "Profile syncing" to "Backup and sync", adding a dedicated settings menu and more (#15003)
...
- fix(multi-srp): display multichain accounts in SRP list (#14724)
- fix(confirmations): remove transaction simulations from wallet initiated send flow (#14994)
...
- fix: update confirmation font sizes (#14715)
...
- fix(bridge): reset state to prevent stale quotes on token switch (#15571)
...
- feat: add `MegaETH Testnet` as default network (#14241)
- feat: add new `completedOnboarding` onboarding state property (#14370)
- feat: Init asset details balance, remove PV flag (#14439)
- feat: util file for phishing detection (#14232)
- feat: upgrade `@testing-library/react-native` to `13.2.0` (#14450)
- feat: add metrics for Alert System (#14246)
- feat: Updated design-tokens package to 7.0 (#14395)
- feat: bump `@metamask/notification-services-controller` to `^5.0.1` (#14296)
- feat: added mmpoly and mmsans font files (#14394)
...
- feat: Support input types on `SnapUIInput` (#14312)
- feat: multichain trx history (#13922)
...
- fix: update `ConfirmAlertModal` copy blockaid (#14206)
- fix: code fence name - show only in Beta (#14490)
...
- fix: Relocate confirmations code to `legacy` folder (#14313)
...
- fix: Remove `Transaction Finalized` event tied to STX (#14293)
- fix: yml formatting causing error (#14277)
...
- fix: cp-7.42.0 fix history transactions loading (#13959)
...
- fix: cp-7.42.0`Tokens` screen performance degradation (#13907)
...
- fix: main balance formatting (#13818)
...
- fix: cp-7.42.0 Disable signature re-designs for ledger account (#13858)
...
- fix: cp-7.42.0 push notification settings bugs (#13772)
...
- [#9001](https://github.com/MetaMask/metamask-mobile/pull/9001): fix:: Reduce enzyme usage in unit test by 25% - [2/3] (#8617, #8823)
